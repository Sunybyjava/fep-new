 ...
 *
 * <p>
 * Description: 负责和前置机通讯，进行报文下发和接收工作；同时对于异常确认，更新终端GPRS通讯的物理地址
 * </p>
 *
 * <p>
...
public class CommunicationScheduler extends Thread {
	...
	public static void main(String[] args) {
		CommunicationScheduler a = new CommunicationScheduler(
				...
				CommunicationServerConstants.COMMSERVICE_COMMUNICATIONSCHEDULER_BatchSave, "-D");
		a.start();
	}

	public CommunicationScheduler(int ListenPort, int MaxConnCount, int ConnTimeOut, int WaitTimeOut230M,
			boolean BatchSave, String Debug_Flag) {
		try {
			...
			try {
				if ((FepLink.GetSocketDownChannel() != null) && (FepLink.GetSocketUpChannel() != null)) {
					...
					System.out.println("Channel Ip  : "
							+ FepLink.GetSocketDownChannel().socket().getInetAddress().getHostAddress());
				}
			} catch (Exception ex) {
			}
		}
	...
	public void WriteTerminalInfotoDataBase() {
		for (Enumeration e = CommunicationServerConstants.TerminalLocalList.elements(); e.hasMoreElements();) {
			TerminalInfo ti = (TerminalInfo) e.nextElement();
			String sSQL = "";
			if (ti.CommuniParamChanged) {
				if (ti.TerminalCurrent == 10) {
					sSQL = "UPDATE RUN_ZDDZXX SET ZDWLDZ='" + ti.TerminalCOMAddress + "',TDIP='" + ti.COMCommunicationIP
							+ "',ZHTBSJ=sysdate WHERE ZDLJDZ='" + ti.TerminalAddress + "' and ZDTXFS=10";
					try {
						if (da.executeUpdate(sSQL) == 0) { // update
															// 不成功，说明数据库没有初始记录，需要插入先
							sSQL = "INSERT INTO RUN_ZDDZXX(ZDLJDZ,ZDTXFS,ZDWLDZ,TDIP,ZHTBSJ,TDDZ) VALUES('"
									+ ti.TerminalAddress + "',10,'1','" + ti.COMCommunicationIP + "',sysdate,'1')";
							da.executeUpdate(sSQL);
						}
					} catch (Exception ex) {
					}
				} else if (ti.TerminalCurrent == 20) {
					sSQL = "UPDATE RUN_ZDDZXX SET ZDWLDZ='" + ti.TerminalCOMAddress + "',TDIP='"
							+ ti.RadioCommunicationIP + "',ZHTBSJ=sysdate WHERE ZDLJDZ='" + ti.TerminalAddress
							+ "' and ZDTXFS=20";
					try {
						if (da.executeUpdate(sSQL) == 0) { // update
															// 不成功，说明数据库没有初始记录，需要插入先
							sSQL = "INSERT INTO RUN_ZDDZXX(ZDLJDZ,ZDTXFS,ZDWLDZ,TDIP,ZHTBSJ,TDDZ) VALUES('"
									+ ti.TerminalAddress + "',20,'1','" + ti.RadioCommunicationIP + "',sysdate,'1')";
							da.executeUpdate(sSQL);
						}
					} catch (Exception ex) {
					}
				} else if (ti.TerminalCurrent == 40) {
					sSQL = "UPDATE RUN_ZDDZXX SET ZDWLDZ='" + ti.TerminalUDPAddress + "',TDIP='" + ti.UDPCommunicationIp
							+ "',TDDZ='" + ti.UDPLocalAddress + "',ZHTBSJ=sysdate WHERE ZDLJDZ='" + ti.TerminalAddress
							+ "' and ZDTXFS=40";
					try {
						if (da.executeUpdate(sSQL) == 0) { // update
															// 不成功，说明数据库没有初始记录，需要插入先
							sSQL = "INSERT INTO RUN_ZDDZXX(ZDLJDZ,ZDTXFS,ZDWLDZ,TDIP,ZHTBSJ,TDDZ) VALUES('"
									+ ti.TerminalAddress + "',40,'" + ti.TerminalUDPAddress + "','"
									+ ti.UDPCommunicationIp + "',sysdate,'" + ti.UDPLocalAddress + "')";
							da.executeUpdate(sSQL);
						}
					} catch (Exception ex) {
					}
				} else if ((ti.TerminalCurrent == 50) || (ti.TerminalCurrent == 51) || (ti.TerminalCurrent == 52)) {
					sSQL = "UPDATE RUN_ZDDZXX SET ZDWLDZ='" + ti.TerminalTCPAddress + "',TDIP='" + ti.TCPCommunicationIp
							+ "',TDDZ='" + ti.TCPLocalAddress + "',ZHTBSJ=sysdate WHERE ZDLJDZ='" + ti.TerminalAddress
							+ "' and ZDTXFS=" + ti.TerminalCurrent;
					try {
						if (da.executeUpdate(sSQL) == 0) { // update
															// 不成功，说明数据库没有初始记录，需要插入先
							sSQL = "INSERT INTO RUN_ZDDZXX(ZDLJDZ,ZDTXFS,ZDWLDZ,TDIP,ZHTBSJ,TDDZ) VALUES('"
									+ ti.TerminalAddress + "'," + ti.TerminalCurrent + ",'" + ti.TerminalTCPAddress
									+ "','" + ti.TCPCommunicationIp + "',sysdate,'" + ti.TCPLocalAddress + "')";
							da.executeUpdate(sSQL);
						}
					} catch (Exception ex) {
					}
				...
				if (ti.TerminalCurrent != 0) {
					...
					CommunicationServerConstants.TerminalLocalList.remove(ti.TerminalAddress);
					CommunicationServerConstants.TerminalLocalList.put(ti.TerminalAddress, ti);
				}
			}
		}
	}
	...
	public void ManualUpdateSwitchMPInfo(int Updatetype, String TerminalAddr, int CLDXH) {
		try {
			CommunicationServerConstants.Trc1.TraceLog("UpdateSwitchMPInfo Start");
			SwitchMPInfo mpi = new SwitchMPInfo();
			ResultSet rset;
			if (Updatetype == 10) { // all
				rset = da.executeQuery(
						"SELECT B.CLDXH,B.CLDDZ,B.CLDBH,A.ZDLJDZ AS MAddrESS,C.ZDLJDZ AS BADDRESS FROM DA_ZDGZ A,DA_CLDXX B,DA_ZDGZ C"
								+ " WHERE B.BYBZ=1 AND A.ZDJH=B.ZDJH AND B.BYZDJH=C.ZDJH AND A.GYH=C.GYH");
			} else {// specified MP
				rset = da.executeQuery(
						"SELECT B.CLDXH,B.CLDDZ,B.CLDBH,A.ZDLJDZ AS MAddrESS,C.ZDLJDZ AS BADDRESS FROM DA_ZDGZ A,DA_CLDXX B,DA_ZDGZ C"
								+ " WHERE B.BYBZ=1 AND A.ZDJH=B.ZDJH AND B.BYZDJH=C.ZDJH AND  B.CLDXH='" + CLDXH
								+ "' AND C.ZDLJDZ='" + TerminalAddr + "'");
			}
			if (!rset.next()) {// have no this record in db,then remove from
								// list
				CommunicationServerConstants.MPInfoList.remove("" + CLDXH + "#" + TerminalAddr);
				return;
			}
			while (!rset.isAfterLast()) {
				try {
					mpi = (SwitchMPInfo) CommunicationServerConstants.MPInfoList.get("" + CLDXH + "#" + TerminalAddr);
					if (mpi != null) {
						mpi.CLDXH = rset.getInt("CLDXH");
						mpi.CLDDZ = (rset.getString("CLDDZ") != null ? rset.getString("CLDDZ") : "");
						mpi.CLDBH = (rset.getString("CLDBH") != null ? rset.getString("CLDBH") : "");
						mpi.MAddress = (rset.getString("MAddress") != null ? rset.getString("MAddress") : "");
						mpi.BAddress = (rset.getString("BAddress") != null ? rset.getString("BAddress") : "");
						CommunicationServerConstants.MPInfoList.remove("" + CLDXH + "#" + TerminalAddr);
						CommunicationServerConstants.MPInfoList.put("" + mpi.CLDXH + "#" + mpi.BAddress, mpi);
					}
					rset.next();
			...
			rset.close();
			CommunicationServerConstants.Trc1.TraceLog("UpdateSwitchMPInfo Finished.");
			rset.close();
		} catch (Exception e) {
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:ManualUpdateSwitchMPInfo error,error is " + e.toString());
		} finally {
			da.close();
		}
	}
	...
	public void ManualUpdateTerminalInfo(int Updatetype, String TerminalAddr) {
		try {
			CommunicationServerConstants.Trc1.TraceLog("UpdateTerminalInfo Start");
			String sOldZDLJDZ = "";
			...
			while (rset.next()) {
				sOldZDLJDZ = (rset.getString("ZDLJDZ") != null ? rset.getString("ZDLJDZ") : "");
				if (sOldZDLJDZ.length() == 0) {
					continue;
				}
				String SIM = (rset.getString("SIMID") != null ? rset.getString("SIMID") : sOldZDLJDZ);
				int ZDSX = 0;
				try {
					ZDSX = Integer.parseInt((rset.getString("ZDSX") != null ? rset.getString("ZDSX") : "01"));
				} catch (Exception ex1) {
				}
				String ZDMM = (rset.getString("ZDMM") != null ? rset.getString("ZDMM") : "8888");
				int JMSF = rset.getInt("JMSF");
				int GYH = rset.getInt("GYH");
				// utils.PrintDebugMessage("sOldZDLJDZ="+sOldZDLJDZ+" GYH="+GYH,
				// "D");
				ti = (TerminalInfo) CommunicationServerConstants.TerminalLocalList.get(sOldZDLJDZ);
				if (ti != null) {
					try {
						if ((ti.TerminalSIMID.length() == 0 || !ti.TerminalSIMID.equals(SIM))
								|| ti.TerminalProperty != ZDSX
								|| (ti.TerminalPassWord.length() == 0 || !ti.TerminalPassWord.equals(ZDMM))
								|| ti.TerminalCodingNo != JMSF || ti.TerminalProtocol != GYH) {
						...
						if (Updatetype == 20) {
							...
							case 10: {
								ti.COMCommunicationIP = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
								break;
							}
							case 20: {
								ti.RadioCommunicationIP = (rset.getString("TDIP") != null ? rset.getString("TDIP")
										: "");
								break;
							}
							case 31: // cmpp
							case 32: // sgip
							case 30: {
								ti.SIMCommunicationIP = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
								break;
							}
							case 40: {
								ti.UDPCommunicationIp = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
								break;
							}
							case 50:
							case 51:
							case 52: {
								ti.TCPCommunicationIp = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
								break;
							}
							default: {
								break;
						...
						CommunicationServerConstants.Log1
								.WriteLog("UpdateTerminal " + sOldZDLJDZ + " Error,Error is " + ex.toString());
					}
				} else {
					// utils.PrintDebugMessage("add
					// ZDLJDZ="+rset.getString("ZDLJDZ")+"
					// GYH="+rset.getInt("GYH"), "D");
					ti = new TerminalInfo();
					ti.TerminalAddress = (rset.getString("ZDLJDZ") != null ? rset.getString("ZDLJDZ") : ""); // 终端逻辑地址
					ti.TerminalSIMID = (rset.getString("SIMID") != null ? rset.getString("SIMID") : ti.TerminalAddress); // 终端SIM卡号码
					ti.TerminalPassWord = (rset.getString("ZDMM") != null ? rset.getString("ZDMM") : "8888"); // 终端密码
					ti.TerminalCodingNo = rset.getInt("JMSF"); // 终端密码算法
					ti.TerminalProtocol = rset.getInt("GYH");
					ti.TerminalProperty = Integer
							.parseInt((rset.getString("ZDSX") != null ? rset.getString("ZDSX") : "01"));
					ti.CommandIndex = 0;
					...
					case 10: {
						ti.COMCommunicationIP = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
						ti.TerminalCOMAddress = (rset.getString("WLDZ") != null ? rset.getString("WLDZ") : "1");
						ti.COMLocalAddress = (rset.getString("TDDZ") != null ? rset.getString("TDDZ") : "1");
						break;
					}
					case 20: {
						ti.RadioCommunicationIP = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
					...
					case 30: {
						ti.SIMCommunicationIP = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
						ti.SIMLocalAddress = (rset.getString("TDDZ") != null ? rset.getString("TDDZ") : "");
						break;
					}
					case 40: {
						ti.UDPCommunicationIp = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
						ti.TerminalUDPAddress = (rset.getString("WLDZ") != null ? rset.getString("WLDZ") : ""); // UDP通讯的物理地址
						ti.UDPLocalAddress = (rset.getString("TDDZ") != null ? rset.getString("TDDZ") : "");
						break;
					}
					case 50:
					case 51:
					case 52: {
						ti.TCPCommunicationIp = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
						ti.TerminalTCPAddress = (rset.getString("WLDZ") != null ? rset.getString("WLDZ") : ""); // TCP通讯的物理地址
						ti.TCPLocalAddress = (rset.getString("TDDZ") != null ? rset.getString("TDDZ") : "");
						break;
					}
					}
					CommunicationServerConstants.TerminalLocalList.put(ti.TerminalAddress, ti);
			...
			CommunicationServerConstants.Trc1.TraceLog("UpdateTerminalInfo Finished.");
			rset.close();
		} catch (Exception e) {
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:ManualUpdateTerminalInfo error,error is " + e.toString());
		} finally {
			da.close();
		}
	}
	...
	public void SleepDownOnLineTerminal() {
		for (Enumeration e = CommunicationServerConstants.TerminalLocalList.elements(); e.hasMoreElements();) {
			TerminalInfo ti = (TerminalInfo) e.nextElement();
			if (((ti.TerminalCurrent == 40) || (ti.TerminalCurrent == 50)) && (ti.OnLineStatus == 20)
				...
				try {
					...
					String sXXBW = "683D003D00684A" + sZDLJDZ.substring(2, 4) + sZDLJDZ.substring(0, 2)
							+ sZDLJDZ.substring(6, 8) + sZDLJDZ.substring(4, 6) + "1004700000080A";
					if (ti.TerminalProperty == 1 || ti.TerminalProperty == 4) { // CDMA终端、模块表
						sXXBW = sXXBW + "04"; // 切换到CDMA睡眠方式
						iTxfs = 50;
					} else if (ti.TerminalProperty == 2 || ti.TerminalProperty == 5) { // GPRS终端、模块表
					...
					sXXBW = sXXBW + ti.TerminalPassWord.substring(2, 4) + ti.TerminalPassWord.substring(0, 2) + "0016"; // 睡眠帧组成完成，进行校验后就加入发送队列即可
					...
					if (sXXBW.length() > 0) {
						...
						if (CommunicationServerConstants.GlobalSendList
		...
		for (Enumeration e = CommunicationServerConstants.TerminalLocalList.elements(); e.hasMoreElements();) {
			...
			try {
				...
				if (ti.TerminalCurrent == 20) {
					fl.TraceLog("230MCommunicationIP is " + ti.RadioCommunicationIP);
				}
				if (ti.TerminalCurrent == 30 || ti.TerminalCurrent == 31 || ti.TerminalCurrent == 32) {
					fl.TraceLog("SIMCommunicationIP is " + ti.SIMCommunicationIP);
					fl.TraceLog("SIMLocalAddress is " + ti.SIMLocalAddress);
				}
				if (ti.TerminalCurrent == 10) {
				...
				if ((ti.TerminalCurrent == 50) || (ti.TerminalCurrent == 51) || (ti.TerminalCurrent == 52)) {
					fl.TraceLog("TerminalTCPAddress is " + ti.TerminalTCPAddress);
					fl.TraceLog("TCPCommunicationIp is " + ti.TCPCommunicationIp);
					fl.TraceLog("TCPLocalAddress is " + ti.TCPLocalAddress);
				}
				if (ti.TerminalCurrent == 40 || ti.TerminalCurrent == 50) {
					fl.TraceLog("LastDataTime is " + formatter.format(ti.LastLinkTime.getTime()));
					fl.TraceLog("OnLineStatus is " + ti.OnLineStatus);
				}
				fl.TraceLog("=========================================================");
			} catch (Exception ex) {
				...
				if ((ti.TerminalCurrent == 40) || (ti.TerminalCurrent == 50)) {
					if (ti.OnLineStatus == 10) { // 判断在心跳间隔内是否有通讯记录，否则认为掉线
						...
						if (c.before(Calendar.getInstance())) {
							utils.PrintDebugMessage("Terminal " + ti.TerminalAddress + " is OffLine Now", Debug);
							ti.OnLineStatus = 20;
					...
					if (ti.OnLineChanged) { // 终端在线状态改变，需要同步数据库信息
						...
						if (ti.OnLineStatus == 20) {
							sSQL = "UPDATE DA_ZDGZ SET YXZT=3 WHERE ZDLJDZ='" + ti.TerminalAddress + "'";
						} else {
							sSQL = "UPDATE DA_ZDGZ SET YXZT=1 WHERE ZDLJDZ='" + ti.TerminalAddress + "'";
						}
						try {
							da.executeUpdate(sSQL);
						} catch (Exception ex) {
							CommunicationServerConstants.Log1
									.WriteLog("Update TerminalOnLineStatue Fail,Error is " + ex.toString());
						}
						String sZDLJDZ = ti.TerminalAddress;
						ti.OnLineChanged = false;
						CommunicationServerConstants.TerminalLocalList.remove(sZDLJDZ);
						CommunicationServerConstants.TerminalLocalList.put(sZDLJDZ, ti);
					}
				}
			} catch (Exception ex) {
				CommunicationServerConstants.Log1.WriteLog("OnLineStatusCheck error, message is " + e.toString());
			}
		}
	}
	...
	private void InitSwitchMeasurePointList() {
		try {
			...
			ResultSet rset = da.executeQuery(
					"SELECT B.CLDXH,B.CLDDZ,B.CLDBH,A.ZDLJDZ AS MAddrESS,C.ZDLJDZ AS BADDRESS FROM DA_ZDGZ A,DA_CLDXX B,DA_ZDGZ C WHERE B.BYBZ=1 AND A.ZDJH=B.ZDJH AND B.BYZDJH=C.ZDJH AND A.GYH=C.GYH");
			if (!rset.next()) {
				return;
			}
			while (!rset.isAfterLast()) {
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:InitSwitchMeasurePointList Port error,error is " + e.toString());
		} finally {
			da.close();
		}
	}
			...
			while (!rset.isAfterLast()) {
				try {
					if (rset.isFirst()) {
						...
						ti.TerminalCodingNo = rset.getInt("JMSF"); // 终端密码算法
						ti.TerminalProperty = Integer
								.parseInt((rset.getString("ZDSX") != null ? rset.getString("ZDSX") : "01")); // 终端属性，01
																												// CDMA终端
																												// 02GPRS
																												// 03230M
						ti.TerminalSIMID = (rset.getString("SIMID") != null ? rset.getString("SIMID") : sOldZDLJDZ); // 终端SIM卡号码
						ti.CommandIndex = 0;
						if (ti.TerminalProperty == 1 || ti.TerminalProperty == 4) { // CDMA终端
							ti.HeartInterval = CommunicationServerConstants.TERMINAL_HEARTINTERVAL_CDMA;
						} else {
					...
					if (sOldZDLJDZ.equals((rset.getString("ZDLJDZ") != null ? rset.getString("ZDLJDZ") : ""))) {
						int i = rset.getInt("TXFS");
						switch (i) { // 根据终端当前通讯方式写入物理地址
						case 10: {
							ti.COMCommunicationIP = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
						...
						case 40: {
							ti.UDPCommunicationIp = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
							ti.TerminalUDPAddress = (rset.getString("WLDZ") != null ? rset.getString("TDDZ") : ""); // UDP通讯的物理地址
							ti.UDPLocalAddress = (rset.getString("TDDZ") != null ? rset.getString("TDDZ") : "");
							ti.OnLineStatus = iYXZT; // 20; //档案初始化默认终端不在线
							break;
						}
						...
						case 52: {
							ti.TCPCommunicationIp = (rset.getString("TDIP") != null ? rset.getString("TDIP") : "");
							ti.TerminalTCPAddress = (rset.getString("WLDZ") != null ? rset.getString("TDDZ") : ""); // TCP通讯的物理地址
							ti.TCPLocalAddress = (rset.getString("TDDZ") != null ? rset.getString("TDDZ") : "");
						...
						ti.CommandIndex = 0;
						ti.TerminalProperty = Integer
								.parseInt((rset.getString("ZDSX") != null ? rset.getString("ZDSX") : "01")); // 终端属性，决定是GPRS、CDMA终端
						if (ti.TerminalProperty == 1 || ti.TerminalProperty == 4) { // CDMA终端
							ti.HeartInterval = CommunicationServerConstants.TERMINAL_HEARTINTERVAL_CDMA;
						} else {
							ti.HeartInterval = CommunicationServerConstants.TERMINAL_HEARTINTERVAL_GPRS;
					...
					CommunicationServerConstants.Log1
							.WriteLog("InitTerminalLocalList Error,Terminal is " + rset.getString("ZDLJDZ"));
				}
			}
			rset.close();
		} catch (Exception e) {
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:InitTerminalLocalList Port error,error is " + e.toString());
		} finally {
			da.close();
		}
	}
	...
	private void registerChannel(Selector selector, SocketChannel channel, int ops) {
		try {
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:registerChannel() error,error is " + ex.toString());
		}
	}

	// 将需要下发的短信组成命令下发到前置机通道上
	private void BuildShortMessageAndSend(SocketChannel channel, int ContentLength, String SendContent,
			String MsgTarget, int MsgType) {
		ByteBuffer buffer = ByteBuffer.allocateDirect(5000); // 用于发送的缓冲
		try {
			byte[] Msg = null;
			CommunicationServerConstants.Trc1.TraceLog("35、Begin to Form up Frame");
			try {
				...
				CommunicationServerConstants.Trc1.TraceLog("36、Frame Build Complete");
			} catch (Exception ex2) {
				CommunicationServerConstants.Log1
						.WriteLog("CommunicationScheduler:GenerateSendMsg() Error,error is " + ex2.toString());
			}
			...
			try {
				...
				CommunicationServerConstants.Log1
						.WriteLog("CommunicationScheduler:PutByteToBuffer() Error,error is " + ex3.toString());
			}
			try {
				channel.write(buffer);
				AckBack = false; // 返回标志
				buffer.clear();
				CommunicationServerConstants.Trc1.TraceLog("37、Write to FepChannel Success " + channel);
			} catch (IOException ex1) {
				buffer.clear();
				// buffer.allocate(5000);
				buffer = ByteBuffer.allocate(5000);
				CommunicationServerConstants.Log1
						.WriteLog("CommunicationScheduler:Fep Link Error,error is " + ex1.toString());
			} finally {
				Sequence = Sequence + 1;
			}
		} catch (Exception ex) {
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:BuildShortMessageAndSend() Error,error is " + ex.toString());
		}
	}

	// 将前置机收到的数据组成命令下发到前置机通道上
	private void BuildUpFepReceiveFrameAndSend(SocketChannel channel, int ContentLength, String SendContent,
			String MsgTarget, String LocalAddr, int FepType, int YXJ) {
		ByteBuffer buffer = ByteBuffer.allocateDirect(5000); // 用于发送的缓冲
		try {
			byte[] Msg = null;
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:BuildUpFepReceiveFrameAndSend() Error,error is " + ex.toString());
		}
	}

	// 将前置机收到的数据组成命令下发到前置机通道上
	private void BuildUpPreReceiveFrameAndSend(SocketChannel channel, String SendContent, String UserNo, int CommandSeq,
			int iFepType) {
		ByteBuffer buffer = ByteBuffer.allocateDirect(5000); // 用于发送的缓冲
		try {
			byte[] Msg = null;
	...
	private String ReBuildFrameOfEncrypt(String Frame, String Ip, String TerminalAddr, int ArithmeticNo) {
		String sResult = "";
		;
		try {
			if (Frame.substring(0, 2).equals("68")) {
				...
				if ((ArithmeticNo == 128 || ArithmeticNo == 129)
						&& (sGNM.equals("04") || sGNM.equals("05") || sGNM.equals("10"))) { // 设置、控制、中继
					...
					if (iTpv == 128) { // 带时间标签
						sMM = Frame.substring(Frame.length() - 20, Frame.length() - 16); // 密码
						sMM = sMM.substring(2, 4) + sMM.substring(0, 2); // 帧中的密码为真实密码的倒置
						sMM = Encode.Encode1(ArithmeticNo, iSEQ, Ip, TerminalAddr, sMM);
						Frame = Frame.substring(0, Frame.length() - 20) + sMM
								+ Frame.substring(Frame.length() - 16, Frame.length());
					} else {
						sMM = Frame.substring(Frame.length() - 8, Frame.length() - 4); // 密码
						sMM = sMM.substring(2, 4) + sMM.substring(0, 2); // 帧中的密码为真实密码的倒置
						sMM = Encode.Encode1(ArithmeticNo, iSEQ, Ip, TerminalAddr, sMM);
						Frame = Frame.substring(0, Frame.length() - 8) + sMM
								+ Frame.substring(Frame.length() - 4, Frame.length());
					}
		...
		try {
			if (sd.SendDirect == 1) { // 发送普通短信的处理
				...
				for (int j = FepCommList.size() - 1; j >= 0; j--) {
					...
					if (l.GetFepType() == sd.TerminalCommunication) {
						...
						if (l.GetFepType() == 20) { // 230M通道的发送
							...
							ti = (TerminalInfo) CommunicationServerConstants.TerminalLocalList.get(sd.TerminalAddress);
							if (ti != null) {
								if (ti.RadioCommunicationIP.length() != 0) {
									if (l.GetSocketDownChannel().socket().getInetAddress().getHostAddress()
											.equals(ti.RadioCommunicationIP)) {
										sd.MessageContent = ReBuildFrameOfEncrypt(sd.MessageContent,
												"192.168.1.217:0000", sd.TerminalAddress, sd.ArithmeticNo);
										sd.MessageLength = sd.MessageContent.length();
										BuildUpFepReceiveFrameAndSend(l.GetSocketDownChannel(), sd.MessageLength,
												sd.MessageContent, sd.MobileNo, "", sd.TerminalCommunication, sd.YXJ); // 组成命令并下发
										break;
									}
								} else {
							...
							BuildShortMessageAndSend(l.GetSocketDownChannel(), sd.MessageLength, sd.MessageContent,
				...
				if (l == null) {
					...
					SaveDownFlowRecordToDB(sd.MobileNo, sd.TerminalCommunication, sd.MessageContent, sd.TerminalAddress,
							l.GetSocketDownChannel());
				}
			} else if (sd.SendDirect == 10 || sd.SendDirect == 20 || sd.SendDirect == 30) // 调用加密机或读卡器
			{
				int iFepType = 0;
				if (sd.SendDirect == 10)
					iFepType = 110;// 国网加密机
				...
				if (l != null) { // 只有找到对应的链路信息后继续处理，否则显示错误信息
					...
					BuildUpPreReceiveFrameAndSend(l.GetSocketUpChannel(), sd.MessageContent, sd.TerminalAddress,
							sd.CommandSeq, iFepType); // 组成命令并下发
					SaveDownFlowRecordToDB("", iFepType, sd.MessageContent, sd.TerminalAddress, l.GetSocketUpChannel()); // 保存通讯记录
				} else {
					// CommunicationServerConstants.Trc1
					// .TraceLog("34、No Correspond FepLink To Send!");
				}
				...
				if (ti != null) { // 找到对应的终端信息后继续，否则提示出错
					...
					for (int j = FepCommList.size() - 1; j >= 0; j--) {
						...
						if ((l.GetSocketDownChannel() != null) && (l.GetFepType() == sd.TerminalCommunication)) {
							// 寻找对应的串口通道
							if (sd.TerminalCommunication == 10 && (l.GetSocketDownChannel().socket().getInetAddress()
									.getHostAddress().equals(ti.COMCommunicationIP))) {
								// CommunicationServerConstants.Trc1
								...
								break;
							} else if ((sd.TerminalCommunication == 40) && (l.GetSocketDownChannel().socket()
									.getInetAddress().getHostAddress().equals(ti.UDPCommunicationIp))) {
								// CommunicationServerConstants.Trc1
								// .TraceLog("32、Find Fep Channel 40!");
								break;
							} else if ((sd.TerminalCommunication == 50) && (l.GetSocketDownChannel().socket()
									.getInetAddress().getHostAddress().equals(ti.TCPCommunicationIp))) {
								// CommunicationServerConstants.Trc1
								// .TraceLog("32、Find Fep Channel 50!");
								break;
							} else if ((sd.TerminalCommunication == 51) && (l.GetSocketDownChannel().socket()
									.getInetAddress().getHostAddress().equals(ti.TCPCommunicationIp))) {
								// CommunicationServerConstants.Trc1
								// .TraceLog("32、Find Fep Channel 51!");
								break;
							} else if ((sd.TerminalCommunication == 52) && (l.GetSocketDownChannel().socket()
									.getInetAddress().getHostAddress().equals(ti.TCPCommunicationIp))) {
								// CommunicationServerConstants.Trc1
					...
					if (l != null) { // 只有找到对应的链路信息后继续处理，否则显示错误信息
						...
						case 10: {
							...
							SaveDownFlowRecordToDB(ti.TerminalCOMAddress, sd.TerminalCommunication, sd.MessageContent,
									ti.TerminalAddress, l.GetSocketDownChannel()); // 保存通讯记录
							break;
						...
						case 30: {
							...
							BuildUpFepReceiveFrameAndSend(l.GetSocketDownChannel(), sd.MessageLength, sd.MessageContent,
									ti.TerminalSIMID, ti.SIMLocalAddress, sd.TerminalCommunication, sd.YXJ);

							SaveDownFlowRecordToDB(ti.TerminalSIMID, sd.TerminalCommunication, sd.MessageContent,
									ti.TerminalAddress, l.GetSocketDownChannel());
						...
						case 40: {
							...
							BuildUpFepReceiveFrameAndSend(l.GetSocketDownChannel(), sd.MessageLength, sd.MessageContent,
									ti.TerminalUDPAddress, ti.UDPLocalAddress, sd.TerminalCommunication, sd.YXJ);
							SaveDownFlowRecordToDB(ti.TerminalUDPAddress, sd.TerminalCommunication, sd.MessageContent,
									ti.TerminalAddress, l.GetSocketDownChannel());
							break;
						}
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:SendDownFlowToFep() Error,SendObject Info is "
							+ sd.TerminalAddress + "," + sd.TerminalCommunication + " CodeLineNo is " + s[0].toString()
...
//			SleepCDMA.add(Calendar.MINUTE, CDMASleepMinute); // 睡眠CDMA终端的时间间隔默认60分钟
			...
			/*
			 * formatter = new SimpleDateFormat("yyyy-MM-dd"); String sDate =
			 * formatter.format(DailySleep.getTime()) + " " +
			 * CommunicationServerConstants.DAILY_SLEEPDO_TIME; formatter1 = new
			 * SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); try { java.util.Date d =
			 * formatter1.parse(sDate); DailySleep.setTime(d); } catch
			 * (Exception ex) { }
			 */
		} catch (Exception ex3) {
		}
		while (true) {
			...
			try {
				try {
					/*
					 * if
					 * (CommunicationServerConstants.DAILY_SLEEPDO.toLowerCase()
					 * .equals( "true")) { Calendar NowDateTime =
					 * Calendar.getInstance(); if
					 * (NowDateTime.after(DailySleep)) { DailySleep(); formatter
					 * = new SimpleDateFormat("yyyy-MM-dd");
					 * DailySleep.add(Calendar.DATE, 1); String sDate =
					 * formatter.format(DailySleep.getTime()) + " " +
					 * CommunicationServerConstants.DAILY_SLEEPDO_TIME;
					 * formatter1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
					 * try { java.util.Date d = formatter1.parse(sDate);
					 * DailySleep.setTime(d); } catch (Exception ex) { } } }
					 */
...
//						ManualUpdateSwitchMPInfo(10, "", 0);
						...
						ManualUpdateTerminalInfo(20, CommunicationServerConstants.TerminalAddr); // 更新指定终端档案
...
//						WriteTerminalInfotoDataBase();
					...
					/*
					 * 20090924 此版本暂时不需要支持CDMA唤醒功能 if
					 * (SleepCDMA.before(Calendar.getInstance())) {
					 * //默认每隔1个小时睡眠不在线CDMA终端 SleepDownOnLineTerminal();
...
//							OnLineStatusCheck();
						...
						/*
						 * for (int i = 0; i < FepCommList.size(); i++) { try {
						 * FepLinkList FepLink = (FepLinkList)
						 * FepCommList.get(i); Calendar c =
						 * FepLink.GetLastCommDate(); //暂时屏蔽对于通道链路超时的判断
						 * c.add(Calendar.MINUTE, TimeOut); //判断链路是否超时 if
						 * (FepLink.GetSocketDownChannel() != null &&
						 * (c.before(Calendar.getInstance()) ||
						 * (!FepLink.ConnectFlag))) { try {
						 ...
						 * FepLink.SetSocketDownChannel(null); } catch
						 * (Exception ex1) { utils.PrintDebugMessage(
						 * "Close Fep UpChannel Error " + ex1.toString(),
						 * Debug); fl.WriteLog(
						 * "CommunicationScheduler:Close Fep UpChannel Error,error message is "
						 * + ex1.toString()); } FepCommList.set(i, FepLink); } }
						 * catch (Exception ex2) { } }
						 */
					}
				} catch (Exception ex1) {
					CommunicationServerConstants.Log1.WriteLog("TimingJob Exception " + ex1.toString());
				}
				SendData();
			} catch (Exception ex4) {
				CommunicationServerConstants.Log1.WriteLog(
						"CommunicationScheduler:FepCommList TimeOut Process Error,error is " + ex4.toString());
			}
			try {
				Thread.sleep(1);
			} catch (Exception ex) {
	...
	private void SendData() {
		...
		try {
			...
			while (iSize > 0) {
				if (AckBack) {
					ReSendTimes = 0;
					sd = (StructSendData) CommunicationServerConstants.GlobalSendList.get(0);
					CommunicationServerConstants.GlobalSendList.remove(0);
					iSize = CommunicationServerConstants.GlobalSendList.size();
					// CommunicationServerConstants.Trc1
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:Process GlobalSendList Error,error is " + ex2.toString());
		}
	}
	...
	private void UpdateLastCommDate(SocketChannel channel) {
		try {
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:UpdateLastCommDate() Error,error is " + ex.toString());
		}
	}
	...
	private void UpdateLastCommDateOfPre(SocketChannel channel) {
		try {
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:UpdateLastCommDateOfPre() Error,error is " + ex.toString());
		}
	}

	private void AckMessage(String CommandID, String sData, SocketChannel channel) {
		ByteBuffer buffer = ByteBuffer.allocateDirect(5000); // 用于发送的缓冲
		try {
			String ss = "0000000D08" + CommandID + sData.substring(16, 24) + "0A";
			buffer.clear();
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:AckMessage() Error,error is " + ex1.toString());
		}
	}

	private void SaveDownFlowRecordToDB(String MsgDst, int ChannelType, String MessageContent, String ZDLJDZ,
		...
		try {
			if (GlobalDownRecordSaveList.size() < CommunicationServerConstants.JMS_MAXCOUNT) {
				...
				CommRecordItem.SetTargetAddress(channel.socket().getInetAddress().getHostAddress());
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:SaveDownFlowRecordToDB() Error,error is " + ex.toString());
		}
	}

	// 对于国网的异常的确认
	private void ResponseToQuanGuoAlarm(int SourceChannel, int MessageLength, String MessageContent) {
		try {
			...
			sd.MessageType = 30;
			sd.MobileNo = "";
			sd.TerminalAddress = MessageContent.substring(16, 18) + MessageContent.substring(14, 16)
					+ MessageContent.substring(20, 22) + MessageContent.substring(18, 20);
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:ResponseToQuanGuoAlarm() Error,error is " + ex.toString());
		}
	}

	// 对于浙江规范的异常的确认
	private void ResponseToZhejiangAlarm(int SourceChannel, int MessageLength, String MessageContent) {
		try {
			String sQrzt, sQrbw = "";
			String sBjbm = MessageContent.substring(36, 40);
			sQrzt = MessageContent.substring(0, 16); // Copy(MessageContent, 1,
														// 16);
			sQrbw = sQrbw + sQrzt;
			...
			sd.MessageType = 30;
			sd.MobileNo = "";
			sd.TerminalAddress = MessageContent.substring(2, 6) + MessageContent.substring(8, 10)
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:ResponseToZhejiangAlarm() Error,error is " + ex.toString());
		}
	}

	@SuppressWarnings({ "unused", "rawtypes" })
	private String SearchTerminalAddress(int SourceChannel, SocketChannel channel, String MsgSrc, String LocalAddr) {
		String sZDLJDZ = "";
		try {
			for (Enumeration e = CommunicationServerConstants.TerminalLocalList.elements(); e.hasMoreElements();) {
				TerminalInfo ti = (TerminalInfo) e.nextElement();
				if (ti.TerminalCurrent == SourceChannel) {
					if (SourceChannel == 10) {
						if (ti.TerminalCOMAddress.equals(MsgSrc) && ti.COMLocalAddress.equals(LocalAddr)
								&& ti.COMCommunicationIP.equals(channel.socket().getInetAddress().getHostAddress())) {
							sZDLJDZ = ti.TerminalAddress;
							break;
						}
					} else if (SourceChannel == 30 || SourceChannel == 31 || SourceChannel == 32) {
						if (ti.TerminalSIMID.equals(MsgSrc) && ti.SIMLocalAddress.equals(LocalAddr)
								&& ti.SIMCommunicationIP.equals(channel.socket().getInetAddress().getHostAddress())) {
							sZDLJDZ = ti.TerminalAddress;
							break;
						}
					} else if (SourceChannel == 40) {
						if (ti.TerminalUDPAddress.equals(MsgSrc) && ti.UDPLocalAddress.equals(LocalAddr)
								&& ti.UDPCommunicationIp.equals(channel.socket().getInetAddress().getHostAddress())) {
							sZDLJDZ = ti.TerminalAddress;
							break;
						}
					} else if (SourceChannel == 50 || SourceChannel == 51 || SourceChannel == 52) {
						if (ti.TerminalTCPAddress.equals(MsgSrc) && ti.TCPLocalAddress.equals(LocalAddr)
								&& ti.TCPCommunicationIp.equals(channel.socket().getInetAddress().getHostAddress())) {
							sZDLJDZ = ti.TerminalAddress;
							break;
						}
					}
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:UpdateTerminalLocalList() Error,error is " + ex.toString());
		}
		return sZDLJDZ;
	}

	private void UpdateTerminalLocalList(String sZDLJDZ, int ZDGY, SocketChannel channel, int SourceChannel,
			String MessageContent, String MsgSrc, String LocalAddr) {
		try {
			TerminalInfo ti = null;
			ti = (TerminalInfo) CommunicationServerConstants.TerminalLocalList.get(sZDLJDZ);
			if (ti != null) {
				switch (SourceChannel) {
				case 10: {
					if (!ti.COMCommunicationIP.equals(channel.socket().getInetAddress().getHostAddress())) {
						ti.CommuniParamChanged = true;
					}
					ti.COMCommunicationIP = channel.socket().getInetAddress().getHostAddress();
					ti.TerminalCOMAddress = MsgSrc;
					ti.COMLocalAddress = LocalAddr;
					ti.TerminalCurrent = SourceChannel;
					CommunicationServerConstants.Trc1.TraceLog("15、UpdateTerminalLocalList ZDLJDZ:" + sZDLJDZ
							+ " Channel:" + SourceChannel + " Address:" + MsgSrc);
					CommunicationServerConstants.TerminalLocalList.remove(sZDLJDZ);
					CommunicationServerConstants.TerminalLocalList.put(sZDLJDZ, ti);
					break;
				}
				...
				case 32:
				case 30: {
					if (!ti.SIMCommunicationIP.equals(channel.socket().getInetAddress().getHostAddress())) {
						ti.CommuniParamChanged = true;
					}
					ti.SIMCommunicationIP = channel.socket().getInetAddress().getHostAddress();
				...
				case 40: {
					ti.UDPCommunicationIp = channel.socket().getInetAddress().getHostAddress();
					...
					if (ti.OnLineStatus == 20) {
						ti.OnLineStatus = 10;
						ti.OnLineChanged = true;
						CommunicationServerConstants.Trc1.TraceLog("19、UpdateTerminalLocalList ZDLJDZ:" + sZDLJDZ
								+ " Channel:" + SourceChannel + " onlien now");
					}
					CommunicationServerConstants.Trc1
							.TraceLog("15、UpdateTerminalLocalList ZDLJDZ:" + sZDLJDZ + " Channel:" + SourceChannel);
					CommunicationServerConstants.TerminalLocalList.remove(sZDLJDZ);
					CommunicationServerConstants.TerminalLocalList.put(sZDLJDZ, ti);
					break;
				}
				...
				case 51:
				case 52: {
					ti.TCPCommunicationIp = channel.socket().getInetAddress().getHostAddress();
					...
					CommunicationServerConstants.Trc1.TraceLog("15、UpdateTerminalLocalList ZDLJDZ:" + sZDLJDZ
							+ " Channel:" + SourceChannel + " TerminalProtocol:" + ti.TerminalProtocol);
					CommunicationServerConstants.TerminalLocalList.remove(sZDLJDZ);
					CommunicationServerConstants.TerminalLocalList.put(sZDLJDZ, ti);
					break;
				}
				...
				switch (SourceChannel) {
				case 10: {
					tiNew.COMCommunicationIP = channel.socket().getInetAddress().getHostAddress();
					tiNew.TerminalCOMAddress = MsgSrc;
					tiNew.CommuniParamChanged = true;
					tiNew.COMLocalAddress = LocalAddr;
					CommunicationServerConstants.TerminalLocalList.put(sZDLJDZ, tiNew);
					NewTerminalList.add(tiNew);
					break;
				}
				case 20: {
					tiNew.RadioCommunicationIP = channel.socket().getInetAddress().getHostAddress();
					tiNew.TerminalCOMAddress = MsgSrc;
					tiNew.CommuniParamChanged = true;
					tiNew.COMLocalAddress = LocalAddr;
					CommunicationServerConstants.TerminalLocalList.put(sZDLJDZ, tiNew);
				...
				case 32:
				case 30: {
					tiNew.SIMCommunicationIP = channel.socket().getInetAddress().getHostAddress();
				...
				case 40: {
					tiNew.UDPCommunicationIp = channel.socket().getInetAddress().getHostAddress();
				...
				case 50: {
					tiNew.TCPCommunicationIp = channel.socket().getInetAddress().getHostAddress();
				...
				CommunicationServerConstants.Trc1.TraceLog("15、NewTerminalAddToLocalList ZDLJDZ:" + sZDLJDZ
						+ " Channel:" + SourceChannel + " Address" + MsgSrc);
			}
		} catch (Exception ex) {
			CommunicationServerConstants.Log1
	...
	private void DealWithUpFrame(int SourceChannel, int MessageLength, String MessageContent, String MsgSrc,
			String LocalAddr, SocketChannel channel) {
		try {
			...
			if (sf != null) { // 解释成功后，根据数据类型不同处理
				...
				if ((iGYH == Glu_ConstDefine.GY_ZD_QUANGUO) || (iGYH == Glu_ConstDefine.GY_ZD_ZHEJIANG)
				...
				// if ((iDataType == Glu_ConstDefine.SJLX_ZDDL || iDataType ==
				// Glu_ConstDefine.SJLX_ZXXT)
				// && (SourceChannel > 10)) {//心跳帧带表地址，其他帧没有
				// //0001000100010012DD1000000000303030303039313230303937
				// 最后8个字节为表地址
				...
				// iIndex + 32).equals("00")) {//accepted (0),rejected-permanent
				// (1),rejected-transient (2)
				...
				// bCallBack = true;
				// }
				// } else if ((rd.DataType == Glu_ConstDefine.SJLX_PTSJ ||
				// rd.DataType == Glu_ConstDefine.SJLX_SZFH)
				// && sa.DealStep == 21) {
				// sa.DealStep = 30;//finish
				// bCallBack = true;
				...
				// }
				// }
				if ((rd.DataType != Glu_ConstDefine.SJLX_ZDTC) && (rd.DataType != Glu_ConstDefine.SJLX_ZDDL)
						&& (rd.DataType != Glu_ConstDefine.SJLX_ZXXT) && (rd.DataType != Glu_ConstDefine.SJLX_WDY)) {
					// 提交到报文处理的数据类型:非终端心跳、登录、退出登录以及未定义的报文
					CommunicationServerConstants.Trc1.TraceLog("13、FrameExplained TerminalLogicAdd:"
							+ rd.TerminalLogicAdd + " CommandSeq:" + sf.CommandSeq + " iDataType:" + iDataType);
	...
	private void DealWithPreUpFrame(int SourceChannel, int MessageLength, String MessageContent, String MsgSrc,
			String LocalAddr, String sUserNo, int CommandSeq, SocketChannel channel, int CommandType) {
		try {
	...
	private int FindSourceChannel(SocketChannel channel) {
		try {
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:FindSourceChannel() Error,error is " + ex.toString());
			return 0;
		}
	}
	...
	private void ACDPosition(String sData) {
		...
		if (iACD == 2) { // 如果请求访问位有效，则说明有异常需要获取
			...
			if (Integer.parseInt(sEC, 16) == 0) {
				...
				sEC = "0" + Integer.toHexString(Integer.parseInt(sEC, 16) - 1).toUpperCase() + sEC;
			} else {
				sEC = Integer.toHexString(Integer.parseInt(sEC, 16) - 1).toUpperCase() + sEC;
			}
			String sXXBW = "6839003900684B" + sData.substring(14, 24) + "0E6000000100" + sEC + "0816";
			...
			sd.MessageType = 30;
			sd.MobileNo = "";
			sd.TerminalAddress = sData.substring(16, 18) + sData.substring(14, 16) + sData.substring(20, 22)
	...
	private void AnalyzeCommandContent(String sData, int CommandType, byte[] data, SocketChannel channel) {
		try {
			...
			if (sData.length() != utils.HexStrToInt(sMessageLen) * 2) {
				...
				case 2: {
					try {
						...
						CommunicationServerConstants.Trc1.TraceLog("12、AnalyzeCommandContent" + " SoruceAddress:"
								+ SourceChannel + " TargerAddress:" + LocalAddr + " MsgSrc:" + MsgSrc);
						DealWithUpFrame(SourceChannel, MessageLength, MessageContent, MsgSrc, LocalAddr, channel); // 提交规约解释处理
						if (SourceChannel == 20) { // 230M通讯类型
							ACDPosition(MessageContent); // 对于230M的通道，需要判断请求访问位，然后下发异常主动读取的命令
						}
					} catch (Exception ex) {
						CommunicationServerConstants.Log1
								.WriteLog("CommunicationScheduler:Process command 2 Error,error is " + ex.toString());
					}
					break;
				}
				...
				case 17: { // 批量提交的数据
					try {
						int MessageCount = 0;
						MessageCount = utils.HexStrToInt(sData.substring(24, 32)); // 本次提交的消息数量
						int TotalLength = 0;
						for (int i = 0; i < MessageCount; i++) {
							int MessageLength = 0;
							String MessageContent = null;
							MessageLength = utils.HexStrToInt(sData.substring(32 + TotalLength, 36 + TotalLength)); // 实际有效内容的长度
							byte[] Content = new byte[MessageLength];
							...
							byte[] Src = new byte[25];
							for (int j = 0; j < 25; j++) {
								Src[j] = data[j + 18 + MessageLength + TotalLength / 2];
							}
							String MsgSrc = new String(Src).trim(); // 消息来源物理地址
							String LocalAddr = "";
							byte[] From = new byte[25];
							for (int j = 0; j < 25; j++) {
								From[j] = data[j + 43 + MessageLength + TotalLength / 2];
							}
							LocalAddr = new String(From).trim();
							CommunicationServerConstants.Trc1.TraceLog(
									"12、AnalyzeCommandContent" + " SoruceAddress:" + SourceChannel + " TargerAddress:"
											+ LocalAddr + " MsgSrc:" + MsgSrc + " MsgContent:" + MessageContent);
							TotalLength = TotalLength + (MessageLength + 52) * 2;
							DealWithUpFrame(SourceChannel, MessageLength, MessageContent, MsgSrc, LocalAddr, channel); // 提交规约解释处理
							if (SourceChannel == 20) { // 230M通讯类型
								ACDPosition(MessageContent); // 对于230M的通道，需要判断请求访问位，然后下发异常主动读取的命令
							}
						...
						CommunicationServerConstants.Log1
								.WriteLog("CommunicationScheduler:Process command 17 Error,error is " + ex.toString());
					}
					break;
				}
				default: {
			...
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:AnalyzeCommandContent() Error,error is " + ex2.toString());
		}
	}

	private void AnalyzePreCommandContent(String sData, int CommandType, byte[] data, SocketChannel channel) {
		try {
			String sMessageLen = "0";
			sMessageLen = sData.substring(1, 8);
			if (sData.length() != utils.HexStrToInt(sMessageLen) * 2) {
				...
				try {
					...
					int CommandSeq = 0;
					CommandSeq = utils
							.HexStrToInt(sData.substring(28 + MessageLength * 2 + 40, 28 + MessageLength * 2 + 40 + 8)); // 实际有效内容的长度
					...
					CommunicationServerConstants.Trc1.TraceLog("12、AnalyzeCommandContent" + " SoruceAddress:"
							+ SourceChannel + " TargerAddress:" + LocalAddr + " sUserNo:" + sUserNo);
					DealWithPreUpFrame(SourceChannel, MessageLength, MessageContent, MsgSrc, LocalAddr, sUserNo,
	...
	private void RegisterFepInfo(String Data, SocketChannel channel) {
		try {
				...
				if (LinkType == 10) { // 上行通道注册
					if (FepLink.GetSocketDownChannel() == null) { // 如果下行通道还没有对象，则只需要处理上行通道的更新或者新增
						if (FepLink.GetSocketUpChannel() != null) {
							if ((FepLink.GetSocketUpChannel().socket().getInetAddress().getHostAddress()
									.equals(channel.socket().getInetAddress().getHostAddress()))
								...
								if (FepLink.GetSocketUpChannel() != null) {
									try {
										...
										CommunicationServerConstants.Log1.WriteLog(
												"CommunicationScheduler:FepLink.GetSocketUpChannel().close IOError,error is "
														+ ex2.toString());
									}
								}
								...
								newLink = false;
								CommunicationServerConstants.Trc1
										.TraceLog("11、Update Fep Info :" + FepType + " LinkType:" + LinkType
												+ " HostAddress:" + channel.socket().getInetAddress().getHostAddress());
							}
						}
					} else {
						if ((FepLink.GetSocketDownChannel().socket().getInetAddress().getHostAddress()
								.equals(channel.socket().getInetAddress().getHostAddress()))
							...
							FepCommList.set(i, FepLink);
							newLink = false;
							CommunicationServerConstants.Trc1.TraceLog("11、Update Fep Info :" + FepType + " LinkType:"
									+ LinkType + " HostAddress:" + channel.socket().getInetAddress().getHostAddress());
						}
					}
			...
			if (newLink) { // 没有找到相应的链路信息，即是新增的链路信息，根据链路类型修改对应的链路
				...
				FepCommList.add(l);
				CommunicationServerConstants.Trc1.TraceLog("11、Add Fep Info :" + FepType + " LinkType:" + LinkType
						+ " HostAddress:" + channel.socket().getInetAddress().getHostAddress());
			}
			CommunicationServerConstants.Trc1.TraceLog("12、RegisterFepInfo FepType:" + FepType + " LinkType:" + LinkType
					+ " HostAddress:" + channel.socket().getInetAddress().getHostAddress());
		} catch (Exception ex) {
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:RegisterFepInfo() Error,error is " + ex.toString());
		}
	}
	...
	private void RegisterPreInfo(String Data, SocketChannel channel) {
		try {
					...
					if ((FepLink.GetSocketUpChannel().socket().getInetAddress().getHostAddress()
						...
						if (FepLink.GetSocketUpChannel() != null) {
							try {
								...
								CommunicationServerConstants.Log1.WriteLog(
										"CommunicationScheduler:PreLink.GetSocketUpChannel().close IOError,error is "
												+ ex2.toString());
							}
						}
						FepLink.SetSocketUpChannel(channel); // 将上行通道的链路修改为本次连接的链路
						...
						newLink = false;
						CommunicationServerConstants.Trc1.TraceLog("11、Update Pre Info :" + FepType + " HostAddress:"
			...
			if (newLink) { // 没有找到相应的链路信息，即是新增的链路信息，根据链路类型修改对应的链路
				...
				// }
				PreCommList.add(l);
				CommunicationServerConstants.Trc1.TraceLog("11、Add Pre Info :" + FepType + " HostAddress:"
						+ channel.socket().getInetAddress().getHostAddress());
			}
			CommunicationServerConstants.Trc1.TraceLog("12、RegisterPreInfo PreType:" + FepType + " HostAddress:"
					+ channel.socket().getInetAddress().getHostAddress());
		} catch (Exception ex) {
			CommunicationServerConstants.Log1
					.WriteLog("CommunicationScheduler:RegisterPreInfo() Error,error is " + ex.toString());
		}
	}
	...
	private void processData(int CommandID, SocketChannel channel, String ReceiveData, byte[] Data) {
		...
		case 257: { // 加密机、读卡器请求连接的消息0x00000101
			CommunicationServerConstants.Trc1.TraceLog("  RegisterPre " + ReceiveData + channel.toString());
			RegisterPreInfo(ReceiveData, channel); // 注册通道，加入队列
			UpdateLastCommDateOfPre(channel); // 更新最近通讯时间
			CommunicationServerConstants.Trc1.TraceLog("  " + PreCommList.size());
			AckMessage(ReceiveData.substring(10, 16), ReceiveData, channel); // 返回确认消息
			CommunicationServerConstants.Trc1.TraceLog("  AckMessage " + ReceiveData + channel.toString());
			break;
		}
		...
		try {
			...
			for (int j = 0; j < FepCommList.size(); j++) {
				...
				if ((l.GetSocketUpChannel() != null) && (l.GetSocketUpChannel() == socketChannel)) {
					...
					CommunicationServerConstants.Trc1.TraceLog("17、Set Null Fep Info :" + l.GetFepType()
							+ " LinkType:10 HostAddress:" + socketChannel.socket().getInetAddress().getHostAddress());
					if (l.GetSocketDownChannel() == null) {
						FepCommList.remove(l);
						CommunicationServerConstants.Trc1
								.TraceLog("FepChannel removed from FepLinkList,FepChannel Type is " + l.GetFepType());
					}
					break;
					...
					CommunicationServerConstants.Trc1.TraceLog("17、Set Null Fep Info :" + l.GetFepType()
							+ " LinkType:20 HostAddress:" + socketChannel.socket().getInetAddress().getHostAddress());
...
//			System.gc();
					...
					if (n == 0) {
						...
						while (it.hasNext()) {
							try {
								...
								CommunicationServerConstants.Log1.WriteLog(
										"CommunicationScheduler:ListenThreadRun() error,error is " + ex5.toString());
							}
						}
					}
				} catch (Exception ex2) {
					CommunicationServerConstants.Log1.WriteLog("ListenThread Error:" + ex2.toString());
				}
			}
		}
	}
				...
				try {
					if (GlobalUPRecordSaveList.size() >= BatchSaveNum) {
						for (int i = 0; i < BatchSaveNum; i++) {
							UpFlowRecordSaveList.add(GlobalUPRecordSaveList.get(i));
						}
						// CommunicationServerConstants.Log1.WriteLog("SaveUpFlowRecordToDB");
						da_SaveRecord.SaveUpFlowRecordToDB(UpFlowRecordSaveList);
					...
					if (GlobalDownRecordSaveList.size() >= BatchSaveNum) {
						for (int i = 0; i < BatchSaveNum; i++) {
							DownFlowRecordSaveList.add(GlobalDownRecordSaveList.get(i));
						}
						da_SaveRecord.SaveDownFlowRecordToDB(DownFlowRecordSaveList);
					...
					CommunicationServerConstants.Log1.WriteLog(
							"CommunicationScheduler:SaveUpAndDownRecordThreadRun() error,error is " + ex.toString());
				}
			}
		}
	}
		...
		public void run() {
			while (true) {
				try {
					...
					CommunicationServerConstants.Log1
							.WriteLog("CommunicationScheduler:SendDLMSThreadRun() error,error is " + ex.toString());
				}
			}
		}
		...
		private void SendDLMSData() {
			try {
				...
				while (iSize > 0) {
					// if (DLMSAckBack) {
					// ReSendTimes = 0;
					sdDLMS = (StructDLMSSendData) CommunicationServerConstants.GlobalDLMSSendList.get(0);
					MessageContent = "";
					...
					case 10:// AARE
						if (sdDLMS.SecurityLevel == 0) {
							...
							String sData = "8002" + Encode.StrStuff("0", 4, sdDLMS.ProVer, 10);
							sData = sData + "A109060760857405080101";
							sData = sData + "8A02" + Encode.StrStuff("0", 4, sdDLMS.ProVer, 10);
							sData = sData + "8B0760857405080201";
							sData = sData + "AC0A8008"
									+ Encode.StringtoHexASCII(Encode.StrStuff("0", 8, sdDLMS.Password, 10));
							sData = sData + "BE0F040D01000000065F0400000019FFFF";
							sData = "60" + Encode.IntToHex("" + (sData.length() / 2), "00") + sData;
							MessageContent = "000100100001" + Encode.IntToHex("" + (sData.length() / 2), "0000")
									+ sData;
							sdDLMS.DealStep = 13;
					...
					case 12:// StoC
						...
						String key = Encode.StrStuff("0", 32, sdDLMS.Password, 10);
						aes.setText(text.toLowerCase(), true);
						aes.setKeyLengthIndex(0);
						aes.setKey(key, true);
						enString = aes.Cipher().toUpperCase();
						MessageContent = "000100100001001F" + "C30181000F0000280000FF01010910" + enString;
						sdDLMS.DealStep = 13;
						sdDLMS.bHaveSend = false;
						break;
					case 20:// Send Data
						...
						break;
					case 22:// Have next
						MessageContent = "0001001000010007" + "C00281"
								+ Encode.IntToHex("" + sdDLMS.DataBlock, "00000000");
						sdDLMS.DealStep = 21;
						sdDLMS.bHaveSend = false;
						break;
					case 30:// Finish
					...
					if (sdDLMS.DealStep % 2 != 0 && !sdDLMS.bHaveSend) {
						...
						sd.YXJ = sdDLMS.YXJ;
						CommunicationServerConstants.Trc1
								.TraceLog("07、DLMSDownFlow Content is TXLX:" + sd.TerminalCommunication + " SJCD:"
										+ sd.MessageLength + " ZDLJDZ:" + sd.TerminalAddress);
						SendDownFlowToFep(sd);
					 ...
					 * (ReSendTimes < 1)) { if (sd != null) { ReSendTimes =
					 * ReSendTimes + 1; SendDownFlowToFep(sd); SendTime =
					 * Calendar.getInstance();
					 * SendTime.add(Calendar.MILLISECOND, 50);
					 * CommunicationServerConstants.Trc1.TraceLog(
					 * "Resend frame:" + sd.MessageContent + " TerminalInfo " +
					 * sd.TerminalAddress + " " + sd.TerminalCommunication);
					 * }else { DLMSAckBack = true; iSize =
					 * CommunicationServerConstants.GlobalDLMSSendList.size(); }
					 * }else
					 */if (SendTime.before(Calendar.getInstance())) {
						iSize = CommunicationServerConstants.GlobalDLMSSendList.size();
						CommunicationServerConstants.GlobalDLMSSendList.remove(0);
					}
					try {
				...
				CommunicationServerConstants.Log1
						.WriteLog("CommunicationScheduler:Process GlobalDLMSSendList Error,error is " + ex2.toString());
			}
		}
	}
	...
	class Send230MThread extends Thread { // 230M发送线程
			...
			while (true) {
				try {
					if (CommunicationServerConstants.Global230MSendList.size() > 0) {
						StructSendData sd = (StructSendData) CommunicationServerConstants.Global230MSendList.get(0);
						if (DataSend(sd)) { // 数据返回或者等待超时
							CommunicationServerConstants.Global230MSendList.remove(sd);
						}
					}
					try {
						Thread.sleep(1);
					...
					CommunicationServerConstants.Log1
							.WriteLog("CommunicationScheduler:Send230MThreadRun() error,error is " + ex.toString());
				}
			}
		}
		...
		public boolean DataSend(StructSendData SendData) { // 发送230M数据
			...
			try {
				try {
					CommunicationServerConstants.Trc1.TraceLog("44、SendDownFlowBy230M Content is "
							+ SendData.TerminalCommunication + " " + SendData.MessageLength + " "
							+ SendData.TerminalAddress + " " + SendData.MessageContent);
					SendDownFlowToFep(SendData); // 此处不进行消息的等待或者超时处理，由230M通道处理消息的返回、超时等
				} catch (Exception ex) {
					CommunicationServerConstants.Log1.WriteLog(
							"CommunicationScheduler:Send230MThreadDataSend() error,error is " + ex.toString());
				}
			} finally {
			}
			return result;
	...
	class TerminalInfo { // 终端逻辑地址和物理地址的对应关系结构
		...
		public int TerminalProperty = 0; // 终端属性，判断是01 CDMA终端 04 CDMA模块表 02
											// GPRS终端 05GPRS模块表 。。。。。
