 ...
 *
 * <p>
 * Description: 负责和主站进行通讯，进行组帧后下发；或者直接下发短信；对于前置机收到的终端原始报文进行分发
 * </p>
 *
 * <p>
...
public class MessageExchange extends Thread {
	...
	/*
	 * 20090924 此版本暂时不采用JMS队列，历史及异常数据直接保存数据库 private HisDataJMSQueue hd = null;
	 * //发送历史数据到JMS的对象 private AlarmDataJMSQueue ad = null; //发送异常数据到JMS的对象
	 */
	...
	public MessageExchange(int ListenPort, int MaxConnCount, int ConnTimeOut, String Debug_Flag) {
		Port = ListenPort;
		...
		try {
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:Init Socket Server Port error,error is " + ex.toString());
		}

		/*
		 * 20090924 此版本暂时不采用JMS队列，历史及异常数据直接保存数据库 ad = new AlarmDataJMSQueue();
		...
		try {
			...
			CommunicationServerConstants.Log1
					.WriteLog("Failed to create FrameDataAreaExplain object:" + ex2.toString());
		}
		// 初始化任务对照队列
	...
	private void SendBackToSourceLink(SocketChannel channel, StructReceiveData rd, int AppID) {
		ByteBuffer buffer = ByteBuffer.allocateDirect(5000); // 接收数据的缓存
		try {
			switch (rd.DataType) {
			// 分发召测返回的历史数据
			case (Glu_ConstDefine.SJLX_LSSJZC): {
				int TotalMessageLength = 12 + 4 + 20 + 5 + 15 + 2 + rd.FrameLength + 4;
			...
			case (Glu_ConstDefine.SJLX_YCSJZC): {
				int TotalMessageLength = 12 + 4 + 20 + 3 + 10 + 15 + 2 + rd.FrameLength + 4;
				TotalMessage = new byte[TotalMessageLength];
				byte[] MsgLength = new byte[4];
				MsgLength = utils.int2byte(TotalMessageLength);
				TotalMessage[0] = MsgLength[0];
				...
				try {
					...
					buffer.put(TotalMessage);
					buffer.flip();
					// CommunicationServerConstants.Trc1.TraceLog("23、SendBackAlarmMsg
					// Host:" +
			...
			case (Glu_ConstDefine.SJLX_PTSJ): {
				...
				try {
					...
					buffer.put(TotalMessage);
					buffer.flip();
					// CommunicationServerConstants.Trc1.TraceLog("23、SendBackNormalMsg
					// Host:" +
					// channel.socket().getInetAddress().
					// getHostAddress());
					channel.write(buffer);
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:SendBackToSourceLink() error,error is " + ex2.toString());
		}
	}

	// 将召测返回的数据提交制定的召测对象，这里需要根据不同的命令类型分别进行组包
	private void SendBackToPreSourceLink(SocketChannel channel, StructReceiveData rd, int AppID) {
		ByteBuffer buffer = ByteBuffer.allocateDirect(5000); // 接收数据的缓存
		try {
			...
			try {
				...
				// Host:" +
				// channel.socket().getInetAddress().
				// getHostAddress()+" FrameLength:"+rd.FrameLength+"
				// FrameData:"+rd.FrameData.trim());
				channel.write(buffer);
				Thread.sleep(1);
				AckBack = false;
			} catch (Exception ex) {
	...
	private void UpdateLastCommDate(SocketChannel channel) {
		try {
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:UpdateLastCommDate() error,error is " + ex.toString());
		}
	}

	// 消息确认函数，组成确认报文后直接发送即可
	private void AckMessage(String CommandID, String sData, SocketChannel channel, byte[] RealData) {
		...
		if (sData.length() < 16) {
			...
			try {
				...
				try {
					channel.write(buffer);
					// CommunicationServerConstants.Trc1.TraceLog("02、Respond
					// Receive Data to MasterStation");
				} catch (IOException ex) {
				...
				CommunicationServerConstants.Log1
						.WriteLog("MessageExchange:AckMessage() Error,error is " + ex1.toString());
			}
		}
	}
	...
	private void UpdateLinkSocketInfo(String ZDLJDZ, int GYH, int MLXH, int FrameCount, SocketChannel channel,
			int TaskNo, int TaskCount, int AlarmCount, int AppID, int MLLX) {
		...
		try {
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:UpdateLinkSocketInfo() error,error is " + ex.toString());
		}
	}
	// }
	...
	private void AddFrameToSendList(StructSendData SendData, int TXCS, int YXJ) {
		try {
			...
			// CommunicationServerConstants.Trc1.TraceLog("06、AddFrameToSendList
			// ListSize " +
			// CommunicationServerConstants.GlobalSendList.size());
		} catch (Exception ex) {
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:AddFrameToSendList() error,error is " + ex.toString());
		}
	}

	// Build fram according to the DataArea, prepare for send it.
	@SuppressWarnings("unused")
	private void FrameDLMSUpNormalFrameAndAddToList(String ZDLJDZ, int GYH, int SJQCD, String GNM, String SJQNR,
			int AppID, int TXCS, int ZNXZ, int MLXH, int YXJ, int SFBH, String SecLevel, String ProVer,
			String Password) {
		try {
			...
			try {
				FrameInfo = FrameExplain.IFE_BuildFrame(ZDLJDZ.toCharArray(), GYH, 1, MLXH, GNM.toCharArray(),
						SJQNR.toCharArray());
			} catch (Exception ex) {
				// CommunicationServerConstants.Trc1.TraceLog("55、BuildFrame
				// Error!");
				CommunicationServerConstants.Log1
						.WriteLog("MessageExchange:BuildFrame error,error is " + ex.toString());
				return;
			}
			if (FrameInfo == null) {
			...
			// CommunicationServerConstants.Trc1.TraceLog("06、AddFrameToDLMSSendList
			// ListSize " + iSize);
		} catch (Exception ex1) {
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:FrameDLMSUpNormalFrameAndAddToList() error,error is " + ex1.toString());
		}
	}

	// 根据数据区组成相应的帧，准备下发
	private void FrameUpNormalFrameAndAddToList(String ZDLJDZ, int GYH, int CLDXH, int CLDLX, int SJQCD, String GNM,
			String SJQNR, int AppID, int TXCS, int ZNXZ, int MLXH, int YXJ, int SFBH) {
		try {
			FE_FrameExplain FrameExplain;
			FrameExplain = new FE_FrameExplain();
			char[] FrameInfo = new char[1500];
			...
			if (FrameInfo == null) {
				// CommunicationServerConstants.Trc1.TraceLog("55、can not build
				// Frame!");
				return;
			}
			// CommunicationServerConstants.Trc1.TraceLog("05、BuildFrame is " +
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:FrameUpNormalFrameAndAddToList() error,error is " + ex1.toString());
		}
	}

	// 根据数据区组成相应的帧，准备下发
	private void BuildProPayFrameAddToList(String ZDLJDZ, int GYH, int CLDXH, int CLDLX, int SJQCD, String GNM,
			String SJQNR, int AppID, int TXFS, int iFSFS, int ZNXZ, int MLXH, String DYIP) {
		try {
	...
	private int GetTerminalCommandNo(String ZDLJDZ, int GYH) {
		try {
			TerminalCommandNoList cn = (TerminalCommandNoList) TerminalCommandList.get(ZDLJDZ);
			if (cn != null) {
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:GetTerminalCommandNo() error,error is " + ex.toString());
			return 0;
		}
	}
	...
	private void SetTerminalOnLineStatus(List TerminalList) {
		try {
			for (int i = 0; i < TerminalList.size(); i++) {
				...
				TerminalInfo ti = (TerminalInfo) CommunicationServerConstants.TerminalLocalList.get(to.TerminalAddress); // 根据终端逻辑地址到Hash表中搜索
				if (ti != null) {
					ti.OnLineStatus = to.OnLineStatus;
					CommunicationServerConstants.TerminalLocalList.remove(to.TerminalAddress);
					CommunicationServerConstants.TerminalLocalList.put(to.TerminalAddress, ti);
					CommunicationServerConstants.Trc1
							.TraceLog("----Terminal " + to.TerminalAddress + " OnLineStaus is " + to.OnLineStatus);
				}
			}
		} catch (Exception ex) {
		}
	...
	private void ProcessTerminalOnLineList(List tempList, SocketChannel channel) {
		...
		if (OutList != null) {
			...
			if ((data != null)) { // 发送数据
				...
				bBuffer.flip();
				CommunicationServerConstants.Trc1.TraceLog(
						"23、SendTerminalOnLineList Host:" + channel.socket().getInetAddress().getHostAddress());
				try {
					channel.write(bBuffer); // 如果在这里出现异常，则说明无法连接成功
				} catch (IOException ex) {
					CommunicationServerConstants.Trc1.TraceLog("SendTerminalOnLineStaute Error is " + ex.toString());
				}
				bBuffer.clear();
			}
			return;
		...
		try {
			...
			if (sData.length() != utils.HexStrToInt(sMessageLen) * 2) {
				...
				case 0x00000004: { // 普通数据召测命令
					...
					int iYXJ = utils.HexStrToInt(sData.substring(90 + iSJQCD * 2, 92 + iSJQCD * 2)); // 230M通讯类型的的优先级
					int iSFBH = utils.HexStrToInt(sData.substring(92 + iSJQCD * 2, 94 + iSJQCD * 2)); // 算法编号，用于加密算法
					// socket报文内容分析完成，更新对应的链路的信息，然后调用组帧后加入发送队列
					// CommunicationServerConstants.Trc1.TraceLog("03、BuildNromalFrame
					// ZDLJDZ:" + sZDLJDZ +
					// " GYH:" + iGYH + " AppID:" + iAppID
					// + " MLXH:" + iMLXH + " GNM:" + sGNM +
					// " DataArea:" + sSJQNR);
					UpdateLinkSocketInfo(sZDLJDZ, iGYH, iMLXH, 1, channel, 0, 0, 0, iAppID, Glu_ConstDefine.SJLX_PTSJ);
					// As to DLMS commands,need to send AARE and HLS
					// authentication before the get/set/action request.
					if (iGYH == Glu_ConstDefine.GY_ZD_DLMS) {
						// The first byte in SJQNR is Security
						// level,0:lowest;1:low;2:highest.
						...
						FrameDLMSUpNormalFrameAndAddToList(sZDLJDZ, iGYH, iSJQCD, sGNM, sSJQNR, iAppID, iTXCS, iZNXZ,
								iMLXH, iYXJ, iSFBH, sSL, sProVer, sPassword);
					} else {
						FrameUpNormalFrameAndAddToList(sZDLJDZ, iGYH, iCLDXH, iCLDLX, iSJQCD, sGNM, sSJQNR, iAppID,
				...
				case 0x00000005: { // 历史数据召测命令处理
					...
					int TaskCount = utils.HexStrToInt(sData.substring(90 + iSJQCD * 2, 92 + iSJQCD * 2));
					int iYXJ = utils.HexStrToInt(sData.substring(92 + iSJQCD * 2, 94 + iSJQCD * 2)); // 230M通讯类型的的优先级
					// socket报文内容分析完成，更新对应的链路的信息，然后调用组帧后加入发送队列
					// CommunicationServerConstants.Trc1.TraceLog("03、BuildHiFrame
					// ZDLJDZ:" + sZDLJDZ +
					// " AppID:" + iAppID + " GYH:" + iGYH +
					// " MLXH:" + iMLXH + " GNM:" + sGNM +
					// " TaskCount:" + TaskCount +
					// " DataArea:" + sSJQNR);
					UpdateLinkSocketInfo(sZDLJDZ, iGYH, iMLXH, 1, channel, 0, 0, TaskCount, iAppID,
					...
					if (iGYH == Glu_ConstDefine.GY_ZD_DLMS) {
						...
						sSJQNR = sSJQNR.substring(14, sSJQNR.length());
						FrameDLMSUpNormalFrameAndAddToList(sZDLJDZ, iGYH, iSJQCD, sGNM, sSJQNR, iAppID, iTXCS, iZNXZ,
								iMLXH, iYXJ, 0, sSL, sProVer, sPassword);
					} else {
						FrameUpNormalFrameAndAddToList(sZDLJDZ, iGYH, iCLDXH, iCLDLX, iSJQCD, sGNM, sSJQNR, iAppID,
				...
				case 0x00000006: { // 异常事件召测命令处理
					...
					int AlarmCount = utils.HexStrToInt(sData.substring(90 + iSJQCD * 2, 92 + iSJQCD * 2));
					int iYXJ = utils.HexStrToInt(sData.substring(92 + iSJQCD * 2, 94 + iSJQCD * 2)); // 230M通讯类型的的优先级
					// socket报文内容分析完成，更新对应的链路的信息，然后调用组帧后加入发送队列
					// CommunicationServerConstants.Trc1.TraceLog("03、BuildEveFrame
					// sZDLJDZ:" + sZDLJDZ +
					// " AppID:" + iAppID + " GYH:" + iGYH +
					// " MLXH:" + iMLXH + " GNM:" + sGNM +
					// " AlarmCount:" + AlarmCount +
					// " DataArea:" + sSJQNR);
					UpdateLinkSocketInfo(sZDLJDZ, iGYH, iMLXH, 1, channel, 0, 0, AlarmCount, iAppID,
							Glu_ConstDefine.SJLX_YCSJZC);
					FrameUpNormalFrameAndAddToList(sZDLJDZ, iGYH, iCLDXH, iCLDLX, iSJQCD, sGNM, sSJQNR, iAppID, iTXCS,
							iZNXZ, iMLXH, iYXJ, 0);
					break;
				}
				case 0x0000000A: { // 下发短信
					...
					// CommunicationServerConstants.Trc1.TraceLog("03、SendShotMsg
					// MobileNo:" + ss.MobileNo +
				...
				case 0x0000000C: { // 下发自定义命令
					...
					int iYXJ = utils.HexStrToInt(sData.substring(84 + iSJQCD * 2, 86 + iSJQCD * 2)); // 230M通讯类型的的优先级
					// socket报文内容分析完成，更新对应的链路的信息，然后直接就可以加入发送队列
					// CommunicationServerConstants.Trc1.TraceLog("03、SendSelfFrame
					// " + sZDLJDZ +
					// " GYH:" + iGYH + " MLXH:" + iMLXH +
				...
				case 0x00000102: { // 调用读卡器
					...
					// + " MLXH:" + iMLXH + " GNM:" + sGNM +
					// " DataArea:" + sSJQNR);
					UpdateLinkSocketInfo(sZDLJDZ, iGYH, iMLXH, 1, channel, 0, 0, 0, iAppID, 0);
					BuildProPayFrameAddToList(sZDLJDZ, iGYH, iCLDXH, iCLDLX, iSJQCD, sGNM, sSJQNR, iAppID, iTXCS, iFSFS,
							iZNXZ, iMLXH, sDYIP);
					break;
				}

					/*
					 * case 0x00000103:{//预付费直接发送 iAppID =
					 * utils.HexStrToInt(sData.substring(24, 32)); //调用的AppID
					 * iTXCS = utils.HexStrToInt(sData.substring(72, 74));
					 * //传入的通讯方式 iZNXZ = utils.HexStrToInt(sData.substring(74,
					 * 76)); //智能选择标志 iCLDXH =
					 * utils.HexStrToInt(sData.substring(76, 78)); //测量点序号
					 * iCLDLX = utils.HexStrToInt(sData.substring(78, 80));
					 * //测量点类型 iGYH = utils.HexStrToInt(sData.substring(80,
					 * 82)); //规约号
					 * 
					 * byte[] Content = new byte[20]; for (int i = 0; i < 20;
					 * i++) { Content[i] = data[i + 16]; } sZDLJDZ = new
					 * String(Content).trim(); iMLXH =
					 * GetTerminalCommandNo(sZDLJDZ, iGYH);
					 * 
					 * Content = new byte[2]; for (int i = 0; i < 2; i++) {
					 * Content[i] = data[i + 41]; } sGNM = new
					 * String(Content).trim();
					 * 
					 * iSJQCD = utils.HexStrToInt(sData.substring(86, 90));
					 * Content = new byte[iSJQCD]; for (int i = 0; i < iSJQCD;
					 * i++) { Content[i] = data[i + 45]; } sSJQNR = new
					 * String(Content).trim(); int iYXJ =
					 * utils.HexStrToInt(sData.substring(90 + iSJQCD * 2, 92 +
					 * iSJQCD * 2)); //230M通讯类型的的优先级 int iSFBH =
					 * utils.HexStrToInt(sData.substring(92 + iSJQCD * 2, 94 +
					 * iSJQCD * 2)); //算法编号，用于加密算法
					 * //socket报文内容分析完成，更新对应的链路的信息，然后调用组帧后加入发送队列
					 * utils.PrintDebugMessage("03、BuildNromalFrame ZDLJDZ:" +
					 * sZDLJDZ + " GYH:" + iGYH + " AppID:" + iAppID + " MLXH:"
					 * + iMLXH + " GNM:" + sGNM + " DataArea:" + sSJQNR, Debug);
					 * UpdateLinkSocketInfo(sZDLJDZ, iGYH, iMLXH, 1, channel, 0,
					 * 0, 0, iAppID, Glu_ConstDefine.SJLX_PTSJ);
					 * FrameUpNormalFrameAndAddToList(sZDLJDZ, iGYH, iCLDXH,
					 * iCLDLX, iSJQCD, sGNM, sSJQNR, iAppID, iTXCS, iZNXZ,
					 * iMLXH, iYXJ, iSFBH); break; }
					 */
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:AnalyzeCommandContent() error,error is " + ex.toString());
		}
	}

	private void processData(int CommandID, SocketChannel channel, String ReceiveData, byte[] Data) {
		UpdateLastCommDate(channel);
		if (CommandID == 0x08000007 || CommandID == 0x08000008 || CommandID == 0x08000009 || CommandID == 0x08000016
				|| CommandID == 0x08000108 || CommandID == 0x08000108) { // 对于提交给实时通讯返回的确认
			AckBack = true;
	...
	/*
	 * private void processData(String sData, SocketChannel channel, byte[]
	 * data) { try { String CommandID = AnalyzeCommandType(sData);
	 * UpdateLastCommDate(channel); int iCommandID = Integer.parseInt(CommandID,
	 * 16); if (iCommandID == 7 || iCommandID == 8 || iCommandID == 9 ||
	 * iCommandID == 22) { //对于提交给实时通讯返回的确认 AckBack = true; } else if
	 * (iCommandID == 4 || iCommandID == 5 || iCommandID == 6 || iCommandID ==
	 * 10 || iCommandID == 12 || iCommandID == 14 || iCommandID == 21 ||
	 * iCommandID == 30 || iCommandID == 31) { AckMessage(CommandID, sData,
	 * channel, data); //有可能跟提交上行数据起冲突，屏蔽回确认 AnalyzeCommandContent(sData,
	 * iCommandID, data, channel); } } catch (NumberFormatException ex) {
	 * fl.WriteLog( "MessageExchange:processData() error,error message is " +
	 * ex.toString()); } }
	 */
	...
	private void receiveData(SelectionKey key) {
		...
		try {
			...
			if (count == 12) {
				...
				// From MasterStation,Message length:"+iLen);
				if (iLen > 500) {
					// if(iLen < 1000){
					buffer[1] = ByteBuffer.allocate(Integer.parseInt(mb.TotalLength, 16) - 12); // 用于接收的缓冲
				...
				if (count >= 0) {
					...
					// System.out.println(mb.CommandID+"
					// "+Integer.parseInt(mb.CommandID, 16));
				}
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:receiveData() IOerror,error message is " + ex3.toString());
		}
	}
	...
	private void DealWithCallBackListData(StructReceiveData rd) {
		...
		try {
				...
				if (sc != null) {
					...
					if (lTemp != null) {
						for (int j = lTemp.size() - 1; j >= 0; j--) {
							TerminalAddCorreCommandSeq ta = (TerminalAddCorreCommandSeq) lTemp.get(j);
							...
							if (rd.TerminalProtocal == Glu_ConstDefine.GY_ZD_QUANGUO
								...
								if ((sTemp.substring(0, 8).equals(rd.TerminalLogicAdd.substring(0, 8)))
									...
									CommunicationServerConstants.Trc1.TraceLog(
											"22、FindRecord ZDLJDZ:" + rd.TerminalLogicAdd + " MLXH:" + rd.CommandSeq
													+ " SJLX:" + rd.DataType + " AppID:" + ta.ApplicationID);
									ta.LastUpdateTime = Calendar.getInstance();
									lTemp.set(j, ta); // 更新链路最后收到的数据时间
									SendBackToSourceLink(sc.GetSocketChannel(), rd, ta.ApplicationID);
									sc.SetTerminalCorreCommandSeqList(lTemp);
									LinkList.remove(sc.GetSocketChannel());
								...
								if ((sTemp.equals(rd.TerminalLogicAdd)) && (ta.CommandSeq[0] == rd.CommandSeq)) {
									...
									ta.LastUpdateTime = Calendar.getInstance();
									lTemp.set(j, ta); // 更新链路最后收到的数据时间
									SendBackToPreSourceLink(sc.GetSocketChannel(), rd, ta.ApplicationID);
									sc.SetTerminalCorreCommandSeqList(lTemp);
									LinkList.remove(sc.GetSocketChannel());
									LinkList.put(sc.GetSocketChannel(), sc);
									FindLinkInfo = true;
					...
					// else if (lTemp != null && lTemp.size() == 0) {
					// //该链路的命令维护队列中没有对象，则认为该链路可以不再使用
					// // CommunicationServerConstants.Trc1.TraceLog("29、This
					// Link have no CallInfo " +
					// // sc.GetSocketChannel());
					// LinkList.remove(sc.GetSocketChannel());
					// sc.GetSocketChannel().close();
					// }
			...
			CommunicationServerConstants.Log1.WriteLog("MessageExchange:DealWithCallBackListData() error,error is "
					+ ex.toString() + " BackData TerminalAddress is " + rd.TerminalLogicAdd);
		}
	}
	/*
	 * //对于上送的数据，根据数据类型进行处理，如果是上送的任务数据或者异常数据，则提交JMS； private void
	 * DealWithJMSListData(StructReceiveData rd) { try {
	 * 
	 * 20090924 此版本暂时不采用JMS队列，历史及异常数据直接保存数据库 if (rd.DataType ==
	 * Glu_ConstDefine.SJLX_YCSJZC || rd.DataType ==
	 * Glu_ConstDefine.SJLX_YCSJZD) { //异常数据提交到异常处理的JMS队列中 StructAutoCommitData
	 * acd = new StructAutoCommitData(); acd.TerminalLogicAdd =
	 * rd.TerminalLogicAdd.toCharArray();
	 * //从终端本地列表中获取终端的真实规约号，避免在类似规约的情况下，采用帧分析后得到的大类规约号，而导致数据无法解释出来 TerminalInfo
	 * ti = (TerminalInfo) CommunicationServerConstants.
	 * TerminalLocalList.get(new String(rd.TerminalLogicAdd).trim()); if (ti !=
	 * null) { acd.TerminalProtocal = ti.TerminalProtocol; } else {
	 * acd.TerminalProtocal = rd.TerminalProtocal; } acd.StationNo =
	 * rd.StationNo; acd.FrameSeq = rd.FrameSeq; acd.CommandSeq = rd.CommandSeq;
	 * acd.DataType = rd.DataType; acd.ControlCode =
	 * rd.ControlCode.toCharArray(); acd.FunctionCode =
	 * rd.FunctionCode.toCharArray(); acd.ManufacturerCode =
	 * rd.ManufacturerCode.toCharArray(); acd.FrameLength = rd.FrameLength;
	 * acd.FrameData = rd.FrameData.toCharArray(); try { ad.sendObject(acd);
	 * //通过已经存在的JMS消息对象发送符合结构要求的数据 } catch (Exception ex1) { ad.close(); ad =
	 * new AlarmDataJMSQueue(); ad.sendObject(acd); utils.PrintDebugMessage(
	 * "90、JMS Object Recreated!", Debug); } ToJMSCount = ToJMSCount + 1;
	 * utils.PrintDebugMessage("22、SendDataToJMSList DataType:" + rd.DataType +
	 * " DataArea: " + rd.FrameData, Debug); } else if (rd.DataType ==
	 * Glu_ConstDefine.SJLX_LSSJZC || rd.DataType ==
	 * Glu_ConstDefine.SJLX_LSSJZD) { //历史数据 StructAutoCommitData acd = new
	 * StructAutoCommitData(); acd.TerminalLogicAdd =
	 * rd.TerminalLogicAdd.toCharArray();
	 * //从终端本地列表中获取终端的真实规约号，避免在类似规约的情况下，采用帧分析后得到的大类规约号，而导致数据无法解释出来 TerminalInfo
	 * ti = (TerminalInfo) CommunicationServerConstants.
	 * TerminalLocalList.get(new String(rd.TerminalLogicAdd).trim()); if (ti !=
	 ...
	 * rd.FunctionCode.toCharArray(); acd.ManufacturerCode =
	 * rd.ManufacturerCode.toCharArray(); acd.FrameLength = rd.FrameLength;
	 * acd.FrameData = rd.FrameData.toCharArray(); try { hd.sendObject(acd); }
	 * catch (Exception ex2) { hd.close(); hd = new HisDataJMSQueue();
	 * hd.sendObject(acd); utils.PrintDebugMessage("91、JMS Object Recreated!",
	 * Debug); } ToJMSCount = ToJMSCount + 1; utils.PrintDebugMessage(
	 * "22、SendDataToJMSList DataType:" + rd.DataType + " DataArea: " +
	 * rd.FrameData, Debug); } } catch (Exception ex) { fl.WriteLog(
	...
	private void InitDeviceAndTaskList() {
		...
		String sSQL1 = "SELECT RWH,RWLX,SJYBS,CLDXH,GYH,SJLX FROM RW_RWXX WHERE (RWLX=1 OR RWLX=2) AND GYH=100 ORDER BY RWH,SJYXH";
		try {
	...
	private void InitCLDInfoList() {
		Glu_DataAccess data = new Glu_DataAccess("./CommService.config");
		try {
			data.LogIn(0);
			ResultSet rs = data.executeQuery(
					"select a.zdljdz,b.cldxh,b.cldbh,b.ctbl,b.ptbl from da_zdgz a,da_cldxx b where a.zdjh=b.zdjh order by a.zdljdz,b.cldxh");
			while (rs.next()) {
				try {
					CLDInfo cld = new CLDInfo();
					cld.CLDID = rs.getInt("CLDBH");
					cld.CLDXH = rs.getInt("CLDXH");
					cld.CT = rs.getInt("CTBL");
					cld.PT = rs.getInt("PTBL");
					cld.ZDLJDZ = rs.getString("ZDLJDZ");
					String sKey = cld.ZDLJDZ + "_" + cld.CLDXH;
					CLDInfoMap.put(sKey, cld);
				} catch (Exception ex) {
			...
			CommunicationServerConstants.Log1.WriteLog("InitCLDInfoList Error,Msg:" + e.toString());
		}
	}
	...
	public void run() {
		...
		js.start();
		int iSendWaitTimeOut = CommunicationServerConstants.COMMSERVICE_MESSAGEEXCHANGE_SENDWAITTIMEOUT;
		int iWaitListMaxCount = CommunicationServerConstants.COMMSERVICE_MESSAGEEXCHANGE_WAITLISTMAXCOUNT;
					...
					if (sc != null) {
						...
						if (d.before(Calendar.getInstance())) {
							try {
								...
								sc.GetSocketChannel().socket().setSoLinger(true, 0);
								AckMessage("0E", "00000001000000010000000100000001", sc.GetSocketChannel(), null);
								sc.GetSocketChannel().close();
							} catch (Exception e3) {
							}
							CommunicationServerConstants.Trc1.TraceLog("80、a link timeout " + sc.GetSocketChannel());
							LinkList.remove(sc.GetSocketChannel());
							break;
						} else { // 对没有超时的连接进行维护
							List lTemp = sc.GetTerminalCorreCommandSeqList();
							if (lTemp != null) {
								if (lTemp.size() >= iWaitListMaxCount) { // 队列维护，超过指定数量清空队列
									...
									for (int i = 0; i < lTemp.size(); i++) {
										TerminalAddCorreCommandSeq ta = (TerminalAddCorreCommandSeq) lTemp.get(i);
	...
	private void AddLinkToList(SocketChannel channel) {
		...
		if (sc == null) {
			...
			sc.SetSocketAddress(channel.socket().getInetAddress().toString());
	...
	private void AddLinkToList(Selector selector, SocketChannel channel) {
		if (channel == null) {
			return;
		}
		// 根据传入的socket链路对象，查找链路维护队列中是否存在，如果不存在则新增
	...
	private void registerChannel(Selector selector, SocketChannel channel, int ops) {
		try {
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:registerChannel() IOerror,error is " + ex.toString());
		}
	}
			...
			for (int k = 0; k < DeviceTaskInfo.size(); k++) {
				...
				if (ti.MeasuredPoint == iCLDXH) {
					for (int i = 0; i < ti.CommandList.size(); i++) {
						...
						// DataProcessLog.WriteLog("比对任务号为：" + ti.TaskNo +
						// "比对数据项为：" + sTemp);
						if (sSJXBS.equals(sTemp)) {
							iTaskNo = ti.TaskNo;
							// DataProcessLog.WriteLog("数据项为：" + sSJXBS + "
							// 的任务数据对应的任务号为：" +
							// iTaskNo);
							break;
						}
	...
	public void UpdateCLDXX(String DataCaption, String DataContent, int CLDBH, String ZDLJDZ, int CLDXZ) {
		String sSQL = "";
		// System.out.println("DataCaption:"+DataCaption);
		if (DataCaption.equals("4A20")) {
			sSQL = "UPDATE RUN_ZTLYC SET DBYXZTZ='" + DataContent + "' WHERE CLDXH=" + CLDBH + " and ZDLJDZ='" + ZDLJDZ
					+ "' and CLDXZ =" + CLDXZ;
		} else if (DataCaption.equals("4A21")) {
			sSQL = "UPDATE RUN_ZTLYC SET DWZTZ='" + DataContent + "' WHERE CLDXH=" + CLDBH + " and ZDLJDZ='" + ZDLJDZ
					+ "' and CLDXZ =" + CLDXZ;
		} else if (DataCaption.equals("4A22")) {
			sSQL = "UPDATE RUN_ZTLYC SET ZXRZTZ='" + DataContent + "' WHERE CLDXH=" + CLDBH + " and ZDLJDZ='" + ZDLJDZ
					+ "' and CLDXZ =" + CLDXZ;
		} else if (DataCaption.equals("4A25")) {
			sSQL = "UPDATE RUN_ZTLYC SET SYSLZTZ='" + DataContent + "' WHERE CLDXH=" + CLDBH + " and ZDLJDZ='" + ZDLJDZ
					+ "' and CLDXZ =" + CLDXZ;
		} else if (DataCaption.equals("4A26")) {
			sSQL = "UPDATE RUN_ZTLYC SET KGZTZ='" + DataContent + "' WHERE CLDXH=" + CLDBH + " and ZDLJDZ='" + ZDLJDZ
					+ "' and CLDXZ =" + CLDXZ;
		}
		try {
	...
	@SuppressWarnings("rawtypes")
	public ArrayList ExtractAlarmZTZ(String DataCaption, String NewZtz, String OldZtz) {
		...
		if ((DataCaption.equals("4A23")) || (DataCaption.equals("4A24"))) {
			...
			for (int i = 7; i >= 0; i--) {
				if (sNewZtz.charAt(i) == '1') {
					sCodeTemp = "C6" + DataCaption.charAt(3) + Integer.toString(8 - i);
					alarmCodeList.add(sCodeTemp);
				}
			}
		} else if ((DataCaption.equals("4A20")) || (DataCaption.equals("4A21")) || (DataCaption.equals("4A22"))
				|| (DataCaption.equals("4A25")) || (DataCaption.equals("4A26"))) {
			sNewZtz = DataSwitch.Fun2HexTo8Bin(NewZtz);
			...
			for (int i = 7; i >= 0; i--) {
				if ((sNewZtz.charAt(i) == '1') && ((sOldZtz.charAt(i) == '0'))) { // 异常发生
					sCodeTemp = "C6" + DataCaption.charAt(3) + Integer.toString(8 - i);
					alarmCodeList.add(sCodeTemp);
				} else if ((sNewZtz.charAt(i) == '0') && ((sOldZtz.charAt(i) == '1'))) { // 异常恢复
					sCodeTemp = "R6" + DataCaption.charAt(3) + Integer.toString(8 - i);
					alarmCodeList.add(sCodeTemp);
				}
			}
		}
	...
	public ArrayList ExtractAlarmData(SFE_AlarmData AlarmData, String ZDLJDZ) {
		...
		if (YCDM.equals("状态字")) {
			sSQL = "SELECT A.DBYXZTZ,A.DWZTZ,A.ZXRZTZ,A.KGZTZ,A.SYSLZTZ,A.CLDXH FROM RUN_ZTLYC A WHERE A.ZDLJDZ='"
					+ ZDLJDZ + "' AND A.CLDXH=" + CLDXH + " AND A.CLDXZ=" + CLDLX;
			ResultSet rset = null;
			try {
				...
				if (!rset.next()) {
					...
					sSQLTemp = sSQLTemp + " values('" + ZDLJDZ + "'," + CLDXH + "," + CLDLX + ",'" + sDbztz + "','"
							+ sDwztz + "','" + sZxztz + "','" + sKgztz + "','" + sSyslztz + "')";
			...
			if (!AlarmData.DataItemList.isEmpty()) {
				...
				for (int i = 0; i < iCount; i++) {
					...
					UpdateCLDXX(sDataCaption, sDataContent, iCldbh, ZDLJDZ, CLDLX);
					alTemp = ExtractAlarmZTZ(sDataCaption, sDataContent, sOldztz);
					if (!alTemp.isEmpty()) {
	...
	@SuppressWarnings({ "unused", "rawtypes" })
	private ArrayList ExtractAlarmDataInfo(SFE_DataListInfo DataListInfo, String ZDLJDZ) {
	...
	private void DealWithFrameData(StructReceiveData rd) {
		try {
			...
			if (rd.DataType == Glu_ConstDefine.SJLX_YCSJZC || rd.DataType == Glu_ConstDefine.SJLX_YCSJZD) {
				// 异常数据提交到异常处理的JMS队列中
				// StructAutoCommitData acd = new StructAutoCommitData();
				// acd.TerminalLogicAdd = rd.TerminalLogicAdd.toCharArray();
				//
				// //System.out.println("====++++++++++DealWithFrameData
				// LJDZ="+acd.TerminalLogicAdd.toString()+"");
				// //从终端本地列表中获取终端的真实规约号，避免在类似规约的情况下，采用帧分析后得到的大类规约号，而导致数据无法解释出来
				// TerminalInfo ti = (TerminalInfo)
				// CommunicationServerConstants.
				// TerminalLocalList.get(new
				// String(rd.TerminalLogicAdd).trim());
				// if (ti != null) {
				...
				// //System.out.println("====++++++++++DealWithFrameData
				// 从TerminalLocalList队列中未取到终端信息");
				// }
				//
				// //System.out.println("====++++++++++DealWithFrameData
				// acd.TerminalProtocal="+Integer.toString(acd.TerminalProtocal));
				//
				//
				...
				// CommunicationServerConstants.Trc1.TraceLog("Call
				// FrameDataAreaExplain, FrameData="
				// + new String(acd.FrameData) + " ProtocolNo=" +
				// acd.TerminalProtocal + " ZDLJDZ=" + new
				// String(acd.TerminalLogicAdd));
				// if(acd.TerminalProtocal == Glu_ConstDefine.GY_ZD_DLMS){
				// DataInfo =
				// FrameDataExplain.IFE_ExplainDataAreaDLMS("".toCharArray(),
				// acd.FrameData, acd.TerminalProtocal);
				// }else{
				// DataInfo =
				// FrameDataExplain.IFE_ExplainDataArea(acd.FrameData,acd.TerminalLogicAdd,
				// acd.TerminalProtocal,acd.ControlCode,acd.FunctionCode);
				// }
				// //dataProcessLog.WriteLog("DataInfo.ExplainResult:" +
				// DataInfo.ExplainResult);
				// //dataProcessLog.WriteLog("DataInfo.DataType:" +
				// DataInfo.DataType);
				// }
				// catch (Exception ex2) {
				// CommunicationServerConstants.Log1.WriteLog("Error to call
				// FrameDataAreaExplain:" + ex2.toString());
				// }
				// //fl.WriteLog("2==解释数据");
				//
				// //<三>、保存数据库
				// try {
				// if ( (DataInfo.ExplainResult == 0) && (DataInfo.DataType ==
				// 30)) {
				...
				// //保存数据库
				// String sZDLJDZ =
				// GetMAddress(AlarmDataList[0].GetMeasuredPointNo(),new
				// String(acd.TerminalLogicAdd));
				//
				// DataAccess_Ala.SaveAlarmData(sZDLJDZ.toCharArray(), DWDM,
				// AlarmDataList, iCount);
				// }
				// else {
				// CommunicationServerConstants.Log1.WriteLog("FrameDataAreaExplain
				// error!ExplainResult:" +
				...
				// }
				// catch (Exception ex1) {
				// CommunicationServerConstants.Log1.WriteLog("Save data error:"
				// + ex1.toString());
				// }
				// //fl.WriteLog("3==保存数据");
				...
				// catch (Exception ex1) {
				//
				// CommunicationServerConstants.Trc1.TraceLog("90、JMS Object
				// Recreated!");
				// }
				// ToJMSCount = ToJMSCount + 1;
				CommunicationServerConstants.Trc1
						.TraceLog("22、SendDataToJMSList DataType:" + rd.DataType + " DataArea: " + rd.FrameData);
			}

			else if (rd.DataType == Glu_ConstDefine.SJLX_LSSJZC || rd.DataType == Glu_ConstDefine.SJLX_LSSJZD) { // 历史数据
				StructAutoCommitData acd = new StructAutoCommitData();
				acd.TerminalLogicAdd = rd.TerminalLogicAdd.toCharArray();
				// 从终端本地列表中获取终端的真实规约号，避免在类似规约的情况下，采用帧分析后得到的大类规约号，而导致数据无法解释出来
				TerminalInfo ti = (TerminalInfo) CommunicationServerConstants.TerminalLocalList
						.get(new String(rd.TerminalLogicAdd).trim());
				if (ti != null) {
					acd.TerminalProtocal = ti.TerminalProtocol;
				} else {
					acd.TerminalProtocal = rd.TerminalProtocal;
				...
				try {
					...
					try {
						...
						DataInfo = FrameDataExplain.IFE_ExplainDataArea(acd.FrameData, acd.TerminalLogicAdd,
								acd.TerminalProtocal, acd.ControlCode, acd.FunctionCode);
						CommunicationServerConstants.Trc1
								.TraceLog("22-1、FrameDataExplain  GYH=" + acd.TerminalProtocal);
					} catch (Exception ex2) {
					...
					// try {
					// CommunicationServerConstants.Trc1.TraceLog("22-2、SaveData
					// ExplainResult="+DataInfo.ExplainResult);
					// if ( (DataInfo.ExplainResult == 0) && (DataInfo.DataType
					// == 20 || DataInfo.DataType == 10)) {
					...
					// HistoryList[i].DataItemAdd(new
					// String(DataItem.GetDataCaption()), new
					// String(DataItem.GetDataContent()));
					// }
					...
					// }
					// if (ti != null) {
					// if (ti.TerminalProperty == 2 || ti.TerminalProperty ==
					// 01) { //只对于终端设备的任务号需要进行重新的匹配
					...
					// String sZDLJDZ =
					// GetMAddress(HistoryList[0].GetMeasuredPointNo(),new
					// String(acd.TerminalLogicAdd));
					// DataAccess_His.SaveHistoryData(sZDLJDZ.toCharArray(),
					// acd.TerminalProtocal,HistoryList,
					// iCount);
					// CommunicationServerConstants.Trc1.TraceLog("22-3、SaveDataSuccess
					// ZDLJDZ:"+sZDLJDZ +"
					// CLDXH:"+HistoryList[0].GetMeasuredPointNo());
					// //将对应的漏点进行更新
					// DataAccess_His.DeleteMissingPoint(acd.TerminalLogicAdd,
					// HistoryList, iCount);
					// //DeleteTaskMissingInfo(s, HistoryList, iCount);
					// }
					// }
					// catch (Exception ex1) {
					// CommunicationServerConstants.Log1.WriteLog("Save data
					// error:" + ex1.toString());
					// }
				} catch (Exception ex2) {
			...
			CommunicationServerConstants.Log1
					.WriteLog("MessageExchange:DealWithJMSListData() error,error is " + ex.toString());
		}
	}
	...
	public String GetMAddress(int CLDXH, String ZDLJDZ) {
		...
		mpi = (SwitchMPInfo) CommunicationServerConstants.MPInfoList.get("" + CLDXH + "#" + ZDLJDZ);
		if (mpi != null) {
	 ...
	 * @version 1.0
	 */
	private class JMSCommitThread extends Thread {
		JMSCommitThread() {
			// 20090924 此版本暂时不采用JMS队列
			/*
			 * JMSConnectTimes = 0;
			...
			while (true) {
				try {
					...
					if (CommunicationServerConstants.GlobalReceiveJMSList.size() > 0) {
						StructReceiveData rd = (StructReceiveData) CommunicationServerConstants.GlobalReceiveJMSList
								.get(0);
				...
				if (n == 0) {
					...
					while (it.hasNext()) {
						try {
							...
							CommunicationServerConstants.Log1
									.WriteLog("MessageExchange:ListenThreadRun() error,error is " + ex5.toString());
						}
					}
				}
			}
