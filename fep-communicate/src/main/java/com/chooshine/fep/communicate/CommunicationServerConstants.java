...
public class CommunicationServerConstants {
	...
	public static String defaultMQHost = "192.168.12.85";
	public static String defaultMQExchange = "EXCHANGE_APP_ROUTE";
	public static String defaultQueue = "APP_ROUTE_QUEUE";
	...
	static void init() {
		...
		try {
			...
			COMMSERVICE_COMMUNICATIONSCHEDULER_PORT = Integer
					.parseInt(((String) prop.getProperty("COMMSERVICE_COMMUNICATIONSCHEDULER_PORT", "5000")).trim());
			COMMSERVICE_MESSAGEEXCHANGE_MAXCOUNT = Integer
					.parseInt(((String) prop.getProperty("COMMSERVICE_MESSAGEEXCHANGE_MAXCOUNT", "100")).trim());
			COMMSERVICE_MESSAGEEXCHANGE_TIMEOUT = Integer
					.parseInt(((String) prop.getProperty("COMMSERVICE_MESSAGEEXCHANGE_TIMEOUT", "10")).trim());
			...
			COMMSERVICE_COMMUNICATIONSCHEDULER_MAXCOUNT = Integer
					.parseInt(((String) prop.getProperty("COMMSERVICE_COMMUNICATIONSCHEDULER_MAXCOUNT", "100")).trim());
			COMMSERVICE_COMMUNICATIONSCHEDULER_TIMEOUT = Integer
					.parseInt(((String) prop.getProperty("COMMSERVICE_COMMUNICATIONSCHEDULER_TIMEOUT", "10")).trim());
			COMMSERVICE_MESSAGEEXCHANGE_WAITLISTMAXCOUNT = Integer.parseInt(
					((String) prop.getProperty("COMMSERVICE_MESSAGEEXCHANGE_WAITLISTMAXCOUNT", "500")).trim());
			COMMSERVICE_MESSAGEEXCHANGE_SENDWAITTIMEOUT = Integer
					.parseInt(((String) prop.getProperty("COMMSERVICE_MESSAGEEXCHANGE_SENDWAITTIMEOUT", "60")).trim());
			COMMSERVICE_MESSAGEEXCHANGE_ENCRYPTORIP = ((String) prop
					.getProperty("COMMSERVICE_MESSAGEEXCHANGE_ENCRYPTORIP", "127.0.0.1")).trim();
			TERMINAL_HEARTINTERVAL_GPRS = Integer
					.parseInt(((String) prop.getProperty("TERMINAL_HEARTINTERVAL_GPRS", "1800")).trim());
			TERMINAL_HEARTINTERVAL_CDMA = Integer
					.parseInt(((String) prop.getProperty("TERMINAL_HEARTINTERVAL_CDMA", "120")).trim());
			JDBC_CONNECTION_URL = ((String) prop.getProperty("JDBC_CONNECTION_URL",
					"jdbc:oracle:thin:@127.0.0.1:1521:grv40")).trim();
			JDBC_CONNECTION_USERNAME = ((String) prop.getProperty("JDBC_CONNECTION_USERNAME", "gd")).trim();
			JDBC_CONNECTION_PASSWORD = ((String) prop.getProperty("JDBC_CONNECTION_PASSWORD", "gd")).trim();
			JMS_URL = ((String) prop.getProperty("JMS_URL", "t3://127.0.0.1:7001")).trim();
			JMS_HISDATAQUEUE = ((String) prop.getProperty("JMS_HISDATAQUEUE", "HistoryDataQueue")).trim();
			JMS_ALARMDATAQUEUE = ((String) prop.getProperty("JMS_ALARMDATAQUEUE", "AlarmDataQueue")).trim();
			
			defaultMQHost = ((String) prop.getProperty("defaultMQHost", "192.168.12.85")).trim();
			defaultMQExchange = ((String) prop.getProperty("defaultMQExchange", "EXCHANGE_APP_ROUTE")).trim();
			defaultQueue = ((String) prop.getProperty("defaultQueue", "APP_ROUTE_QUEUE")).trim();
			MQPort = Integer.parseInt(((String) prop.getProperty("MQPort", "5672")).trim());
			 ...
			 * prop.getProperty("DAILY_SLEEPDO", "false").
			 * toLowerCase()).trim(); DAILY_SLEEPDO_TIME =((String)
			 * prop.getProperty("DAILY_SLEEPDO_TIME", "23:15:00").trim());
			 * DAILY_SLEEP_IPRULE =((String)
			 * prop.getProperty("DAILY_SLEEP_IPRULE", "1.")).trim();
			 */
			String sBatchSave = ((String) prop.getProperty("COMMSERVICE_COMMUNICATIONSCHEDULER_BatchSave", "true"))
					.trim();
			UPDATEFRAME_UDPPORT = Integer.parseInt(((String) prop.getProperty("UPDATEFRAME_UDPPORT", "9999")).trim());
			UPDATEFRAME_UDPIP = ((String) prop.getProperty("UPDATEFRAME_UDPIP", "127.0.0.1")).trim();
			...
			String sBoolean = ((String) prop.getProperty("IRANToGregorian", "false")).trim();
