...
public class SFE_DataItemInfo {// 数据项信息
		...
		try {
			try {
				if (FStorageOrder == CCSX_NX) {// 存储顺序如果是逆序
					FPhysicalData = DataSwitch
							.ReverseStringByByte(FPhysicalData);
				}
				if (PhysicalDataLegalCheck(FPhysicalData, FDataType, FDataLen,
						FStorageType)
						|| (FDataFormat.toUpperCase().equals("X"))) {// 合法性检验
					if (FStorageType == CCFS_BCD) {// BCD码
						switch (FDataType) {
						case c_Float:
							FLogicData = DataSwitch.BCDToFloat(FPhysicalData,
									FDataFormat);
							break;
						case c_DateTime:
							FLogicData = DataSwitch.BCDToDateTime(
									FPhysicalData, FDataFormat);
							break;
						case c_complex:
							FLogicData = ExplainComplexDataType(FPhysicalData,
									FDataFormat);
							break;
						case c_String:
							FLogicData = DataSwitch.HexToString(FPhysicalData,
									FDataFormat);
							break;
						case c_FloatEx:
							FLogicData = DataSwitch.BCDToFloat(FPhysicalData,
						...
						switch (FDataType) {
						case c_Float:
							FLogicData = DataSwitch.HexToInt(FPhysicalData,
									FDataFormat);
							break;
						case c_FloatHex:
						case c_FloatLen:
						case c_FloatNoLx:
							FLogicData = DataSwitch.HexToIntDot(FPhysicalData,
									FDataFormat);
							break;
						case c_FloatBit:
							FLogicData = DataSwitch.HexToBitFloat(
									FPhysicalData, FDataFormat);
							break;
						case c_DateTimeHex:
						case c_DateTimeNoLx:
							FLogicData = DataSwitch.HexToDateTime(
									FPhysicalData, FDataFormat);
							break;
						case c_String:
							FLogicData = DataSwitch.HexToString(FPhysicalData,
						...
						case c_complex:
							...
							break;
						case c_complexEX:
							FLogicData = ExplainComplexEXDataType(
									FPhysicalData, FDataFormat);
							break;
						case c_NOLenStr:
							FLogicData = FPhysicalData;
				...
				System.out.println("数据区解析PhysicalDataToLogicData出错:"
						+ e.toString());
				Glu_ConstDefine.Log1
						.WriteLog("报错函数:数据区解析SFE_DataItemInfo__PhysicalDataToLogicData();报错原因:"
								+ e.toString());
				Result = false;
			}
		} finally {
			...
			while (DataFormat.length() > 0) {
				try {
					...
					if (iPos != -1) {
						...
						DataFormat = DataFormat.substring(iPos + 1,
								DataFormat.length());
					} else {// 最后一个
						sSJGSJ = DataFormat;
						DataFormat = "";
					...
					if (sTemp.equals("0") || sTemp.equals("C")) {
						...
						iDataType = c_FloatEx;
						iDataLen = 10;
					} else if (sTemp.equals("Y") || sTemp.equals("M")
							|| sTemp.equals("D") || sTemp.equals("H")
							|| sTemp.equals("N") || sTemp.equals("S")) {
					...
					if (PhysicalDataLegalCheck(sPhysicalData, iDataType,
							iDataLen, iStorageType)) {// 合法性检验
						if (iStorageType == CCFS_BCD) {// BCD码
							switch (iDataType) {
							case c_Float:
								sLogicData = DataSwitch.BCDToFloat(
										sPhysicalData, sSJGS);
								break;
							case c_DateTime:
								sLogicData = DataSwitch.BCDToDateTime(
										sPhysicalData, sSJGS);
								break;
							case c_String:
								sLogicData = DataSwitch.HexToString(
										sPhysicalData, sSJGS);
								break;
							case c_FloatEx:
								sLogicData = DataSwitch.BCDToFloat(
							...
							switch (iDataType) {
							case c_Float:
								sLogicData = DataSwitch.HexToInt(sPhysicalData,
										sSJGS);
								break;
							case c_String:
								sLogicData = DataSwitch.HexToString(
							...
							sLogicData = DataSwitch
									.HexASCIIToString(sPhysicalData);
						}
					}
					if (sResult.equals("")) {
					...
					System.out.println("数据区解析ExplainComplexDataType出错:"
							+ e.toString());
					Glu_ConstDefine.Log1
							.WriteLog("报错函数:数据区解析SFE_DataItemInfo__ExplainComplexDataType();报错原因:"
									+ e.toString());
					return sResult = "";
				}
			}
		...
		try {
			...
			while (DataFormat.length() > 0) {
				try {
					...
					sSJGSJ = sSJGSJ.substring(iPos + 1, sSJGSJ.length());
					iStorageOrder = Integer.parseInt(sSJGSJ);// 存储顺序
					if (iDataType == c_FloatHex || iDataType == c_FloatLen
							|| iDataType == c_FloatNoLx
							|| iDataType == c_FloatBit) {
						iPos = sSJGS.indexOf('.', 0);
						if (iPos != -1) {// 浮点数
							iDataLen = sSJGS.length() - 1;
						}// 整型
					...
					if (PhysicalDataLegalCheck(sPhysicalData, iDataType,
						...
						if (iStorageType == CCFS_BCD) {// BCD码
							...
							case c_FloatLen:
							case c_FloatNoLx:
								sLogicData = DataSwitch.HexToIntDot(
										sPhysicalData, sSJGS);
								break;
							case c_FloatBit:
								sLogicData = DataSwitch.HexToBitFloat(
										sPhysicalData, sSJGS);
								break;
							case c_DateTimeHex:
								sLogicData = DataSwitch.HexToDateTime(
										sPhysicalData, sSJGS);
								break;
							case c_String:
								sLogicData = DataSwitch.HexToString(
					...
					System.out.println("ExplainComplexEXDataType Error:"
							+ e.toString());
					Glu_ConstDefine.Log1
							.WriteLog("ErrorFunc:SFE_DataItemInfo__ExplainComplexEXDataType();Error:"
									+ e.toString());
					return sResult = "";
				}
			}
			...
			try {
				if (LogicDataLegalCheck(FDataType, FStorageType, FPhysicalData)) {// 合法性检验
					if (FStorageType == CCFS_BCD) {// BCD码
						switch (FDataType) {
						case c_Float:
							FPhysicalData = DataSwitch.FloatToBCD(FLogicData,
									FDataFormat);
							break;
						case c_DateTime:
							FPhysicalData = DataSwitch.DateTimeToBCD(
									FLogicData, FDataFormat);
							break;
						default:
							FPhysicalData = "";
						}
						...
						switch (FDataType) {
						case c_Float:
							FPhysicalData = DataSwitch.IntToHex(FLogicData,
									FDataFormat);
							break;
						case c_String:
							FPhysicalData = DataSwitch.HexToString(FLogicData,
									FDataFormat);
							break;
						case c_NOLenStr:
							FPhysicalData = FLogicData;
							break;
						default:
							FPhysicalData = "";
						}
				...
				System.out.println("数据区解析LogicDataToPhysicalData出错:"
						+ e.toString());
				Glu_ConstDefine.Log1
						.WriteLog("报错函数:数据区解析SFE_DataItemInfo__LogicDataToPhysicalData();报错原因:"
								+ e.toString());
				Result = false;
			}
		} finally {
	...
	public boolean PhysicalDataLegalCheck(String PhysicalData, int DataType,
			int DataLen, int StorageType) {// 物理数据合法性检验
		boolean Result = true;
		int iCheck = 0;
		try {
			try {
				if ((PhysicalData.length() == DataLen * 2)
						|| (DataType == c_NOLenStr)) {// 数据长度校验
					for (int i = 0; i < PhysicalData.length(); i++) {
						iCheck = Integer.parseInt(
								PhysicalData.substring(i, i + 1), 16);
						if ((StorageType == CCFS_BCD)
								&& ((iCheck < 0) || (iCheck > 9))) {// 存储方式等于BCD码
							if (((DataType == c_FloatEx)
									|| (DataType == c_FloatFH) || (DataType == c_IntZJDWFH))
									&& (i != 0)) {// 第一位是符号位，有可能不是BCD码
								Result = false;
								break;
						...
						} else if (((StorageType == CCFS_HEX) || (StorageType == CCFS_ASC))
								&& ((iCheck < 0) || (iCheck > 15))) {// 存储方式等于Hex码或ASCII
							break;
						}
					}
				}
				...
				System.out.println("数据区解析PhysicalDataLegalCheck出错:"
						+ e.toString());
				Glu_ConstDefine.Log1
						.WriteLog("报错函数:数据区解析SFE_DataItemInfo__PhysicalDataLegalCheck();报错原因:"
								+ e.toString());
				Result = false;
			}
		} finally {
		...
		try {
			try {
				for (int i = 0; i < sLogicData.length(); i++) {
					...
					} else if ((iDataType == c_FloatFH)
							|| (iDataType == c_FloatEx)
							|| (iDataType == c_IntZJDWFH)) {
						if (((cCheck < '0') || (cCheck > '9'))
								&& ((cCheck != '.') || (cCheck != '+') || (cCheck != '-'))) {
							return Result;
						}
					} else if ((iDataType == c_String)) {
						if ((iStorageType != CCFS_ASC)
								&& ((cCheck < '0')
										|| ((cCheck > '9') && (cCheck < 'A')) || (cCheck > 'F'))) {
							return Result;
						}
					} else {// 时间型和整型
						if ((cCheck < '0') || (cCheck > '9')) {
				...
				System.out
						.println("数据区解析LogicDataLegalCheck出错:" + e.toString());
				Glu_ConstDefine.Log1
						.WriteLog("报错函数:数据区解析SFE_DataItemInfo__LogicDataLegalCheck();报错原因:"
								+ e.toString());
				Result = false;
			}
		} finally {
