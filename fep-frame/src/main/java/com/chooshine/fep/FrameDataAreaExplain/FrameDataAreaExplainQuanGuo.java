...
public class FrameDataAreaExplainQuanGuo { // 国网数据区解帧类
	...
	public boolean InitialList(String path, URI ur) {
		...
		try {
			try {
				/*
				 * Calendar cLogTime=Calendar.getInstance(); SimpleDateFormat
				 * formatter=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); String
				 * KSSJ=""; String JSSJ=""; cLogTime=Calendar.getInstance();
		...
		try {
			try {
				...
				if (ControlCode.equals("0A") || ControlCode.equals("0C")
					 ...
					 * else if (TermialProtocolType ==
					 * Glu_ConstDefine.GY_ZD_TIANJIN) { //终端天津模块表
			...
			try {
				...
				if (Fn == 1 || Fn == 2) {
					...
					while (FrameDataArea.length() > 0) {
						sDA = FrameDataArea.substring(0, 4);
						sDT = FrameDataArea.substring(4, 8);
						iSetResult = Integer.parseInt(
								FrameDataArea.substring(8, 10), 16); // 错误码
						MeasuredSignList = GluMethodQuanGuo
								.ExplainMeasuredPointList(sDA,
										TermialProtocolType); // 解释信息点得到测量点列表
						CommandList = GluMethodQuanGuo.ExplainCommandList(sDT,
				...
				while (FrameDataArea.length() >= 8) { // 无数据体时至少还4个字节(数据单元标识)
					...
					for (int i = 0; i < MeasuredSignList.length; i++) { // 测量点个数
						...
						if (iDataType == 21) { // 一类数据的小时冻结数据
							...
							sTaskDateTimeInfo = GetTaskDateTimeInfo(
									FrameDataArea.substring(0, 14), iDataType,
									TermialProtocolType);
						} else if (iDataType == 23 || iDataType == 26) { // 日冻结数据
							sTaskDateTimeInfo = GetTaskDateTimeInfo(
									FrameDataArea.substring(0, 6), iDataType,
									TermialProtocolType);
						} else if (iDataType == 24) { // 月冻结数据
							sTaskDateTimeInfo = GetTaskDateTimeInfo(
									FrameDataArea.substring(0, 4), iDataType,
									TermialProtocolType);
						}
						...
						for (int j = 0; j < CommandList.size(); j++) { // 命令个数
							...
							if (ControlCode.equals("0D")
								...
								iCount = Integer.parseInt(
										FrameDataArea.substring(0, 2), 16); // N次谐波
								FrameDataArea = FrameDataArea.substring(2,
										FrameDataArea.length());
							}
							if (ControlCode.equals("0C")
								...
								if (iFn == 57) {
									iCount = Integer.parseInt(
											FrameDataArea.substring(0, 2), 16) - 2; // N次谐波,没有总
								} else if (iFn == 58) {
									iCount = Integer.parseInt(
											FrameDataArea.substring(0, 2), 16) - 1; // N次谐波
								}
								FrameDataArea = FrameDataArea.substring(2,
										FrameDataArea.length());
							}
							for (int m = 0; m < iDataCount; m++) { // 数据点数
								if (MeasuredSignList.length == 1) {
									...
									if (iDataType == 23
											|| (iDataType == 26 && Glu_ConstDefine.BillingDateAddOneDay)) {
										HistoryDataListTemp[m]
									...
									} else if (iDataType == 26
											&& !Glu_ConstDefine.BillingDateAddOneDay) {
										HistoryDataListTemp[m]
												.SetTaskDateTime(DataSwitch
														.IncreaseDateTime(
																sTaskDateTime,
																1 * m, // //统一曲线的起始时间就是每隔小时的0分作为开始
																4));
	...
	public SFE_DataListInfo ExplainAlarmDataAreaQuanGuo( // 终端全国告警数据区解释
		...
		try {
			try {
				// SFE_ExplainAlarmData ExplainAlarmData = new
				// SFE_ExplainAlarmData();
		...
		try {
			try {
				for (int i = 0; i < AlarmDataCount; i++) {
					int iAlarmCode = Integer.parseInt(
							sDateContent.substring(0, 2), 16); // 告警编码
					int iAlarmlen = Integer.parseInt(
							sDateContent.substring(2, 4), 16) + 2; // 告警长度
					String sAlarmDateTime = "";
					if (Glu_ConstDefine.IRANToGregorian) {
						sAlarmDateTime = DataSwitch
								.ReverseStringByByte(sDateContent.substring(4,
										14))
								+ "00"; // 告警发生时间
						sAlarmDateTime = sAlarmDateTime.substring(0, 6) + "01"
								+ sAlarmDateTime.substring(6, 12);
						sAlarmDateTime = Glu_ConstDefine.Cc1
								.TOU_IRANToGregorian(sAlarmDateTime);
						sAlarmDateTime = "20" + sAlarmDateTime.substring(0, 6)
								+ sAlarmDateTime.substring(8, 14);
					} else {
						sAlarmDateTime = "20"
								+ DataSwitch.ReverseStringByByte(sDateContent
										.substring(4, 14)) + "00"; // 告警发生时间
					}
					sDateContent = sDateContent.substring(14,
							sDateContent.length());
					...
					if (iAlarmCode == 1) { // ERC1:数据初始化和版本变更记录
						...
						sPhysicalData = ""
								+ ((Integer
										.parseInt(sDateContent.substring(0,
						...
						if (GYH == Glu_ConstDefine.GY_ZD_698) {
							...
							DataItemInfo = new SFE_DataItemInfo(); // ArrayList里的元素每次ADD都要初始化
							sDataCaption = "EC" + sAlarmCode + "04"; // 发生时的Ub
																		// 数据格式跟上一个一样
							sPhysicalData = sDateContent.substring(0,
									iDataLen * 2);
							sDateContent = sDateContent.substring(iDataLen * 2,
							...
							DataItemInfo = new SFE_DataItemInfo(); // ArrayList里的元素每次ADD都要初始化
							sDataCaption = "EC" + sAlarmCode + "05"; // 发生时的Uc/Ucb
																		// 数据格式跟上一个一样
							sPhysicalData = sDateContent.substring(0,
									iDataLen * 2);
							sDateContent = sDateContent.substring(iDataLen * 2,
							...
							DataItemInfo = new SFE_DataItemInfo(); // ArrayList里的元素每次ADD都要初始化
							sDataCaption = "EC" + sAlarmCode + "07"; // 发生时的Ib
																		// 数据格式跟上一个一样
							sPhysicalData = sDateContent.substring(0,
									iDataLen * 2);
							sDateContent = sDateContent.substring(iDataLen * 2,
							...
							DataItemInfo = new SFE_DataItemInfo(); // ArrayList里的元素每次ADD都要初始化
							sDataCaption = "EC" + sAlarmCode + "08"; // 发生时的Ic
																		// 数据格式跟上一个一样
							sPhysicalData = sDateContent.substring(0,
									iDataLen * 2);
							sDateContent = sDateContent.substring(iDataLen * 2,
						...
						case 255: {
							sAlarmCode = "C0"
									+ ("" + iAlarmCode)
									+ DataSwitch
											.IntToHex("" + iAlarmType, "00");
							break;
						}
						default: {
							sAlarmCode = "R0" + ("" + iAlarmCode);
							sAlarmCode = sAlarmCode
									+ DataSwitch.IntToHex(
											"" + (iAlarmType - 1), "00");
							break;
						}
						...
						sAlarmCode = "C0"
								+ ("" + iAlarmCode)
								+ DataSwitch.StrStuff("0", 2, "" + iAlarmType,
										10);
						AlarmCodeList.add(sAlarmCode);
					}
			...
			try {
				...
				if (DateType == 21) { // 小时冻结数据时标
					if (TermialProtocolType == Glu_ConstDefine.GY_ZD_698) {
						...
						iDataDensity = (Integer.parseInt(
								sDateTimeLabel.substring(2, 4), 16));
					} else {
						sDateTime = Integer.toString((Integer.parseInt(
								sDateTimeLabel.substring(0, 1), 16) & 3))
								+ sDateTimeLabel.substring(1, 2); // 小时
						iDataDensity = (Integer.parseInt(
								sDateTimeLabel.substring(0, 1), 16) & 12) / 4;
					}
					// 获取当前时间
					...
					if (Glu_ConstDefine.IRANToGregorian) {
						sDateTime = DataSwitch
								.ReverseStringByByte(sDateTimeLabel.substring(
										0, 10))
								+ "00"; // 年月日时分
						// Change Iran time to Gregorian
						String sTime = sDateTime.substring(0, 6) + "01"
								+ sDateTime.substring(6, 12);
						Glu_ConstDefine.Cc1.TOU_IRANToGregorian(sTime);
						sNowDateTime = "20" + sNowDateTime
								+ sTime.substring(0, 6)
								+ sTime.substring(8, 14);
					} else {
						sDateTime = "20"
					...
					iDataDensity = Integer.parseInt(
							sDateTimeLabel.substring(10, 12), 16); // 数据密度
					sDataCount = DataSwitch.HexToInt(
							sDateTimeLabel.substring(12, 14), "00"); // 数据点数
					// 当数据点数为个位数时需要补足为1个字节
							...
							if ((iAlarmSign & iPower2) == iPower2) { // 取单个字节每位的值
								if (AlarmType == 1) {
									...
									int iTemp = Integer.parseInt(
											AlarmSign.substring(2, 4), 16); // 变位后状态
