...
public class FrameDataAreaExplainGluMethod { // 数据区解析公用函数类
	...
	public SPE_CommandInfoList InitialCommandInfoList(int ProtocolType) { // 初始化命令信息队列
		...
		try {
			try {
				/*
				 * String sSQL1 = ""; String sSQL2 =
				 * " AND a.CSLX=10 ORDER BY a.SJYBS,a.CSLX,a.SJXH"; String
				 * sSqlStr = ""; if (ProtocolType >= 11 && ProtocolType <= 22) {
				 * //电表规约 sSQL1 =
				 * "SELECT a.SJYBS,a.SJXBS,a.SJXH,a.CSLX,a.SJLX,a.SJGS,a.SJCD,a.CCSX,a.CCLX FROM DATA_ITEM_DB a WHERE a.GYH="
				 * ; } else { //终端规约 sSQL1 =
				 * "SELECT a.SJYBS,a.SJXBS,a.SJXH,a.CSLX,a.SJLX,a.SJGS,a.SJCD,a.CCSX,a.CCLX FROM DATA_ITEM_ZD a WHERE a.GYH="
				...
				if (ProtocolType == Glu_ConstDefine.GY_ZD_DLMS) {// Profile data
																	// of dlms
					sSQL3 = " UNION "
				...
				Statement stmt = conn.createStatement(
						...
						ResultSet.CONCUR_READ_ONLY); // 创建一个可以滚动的只读的SQL语句对象
				...
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainGluMethod__InitialCommandInfoList();Error:"
								+ e.toString());
			}
		} finally {
		}
	 ...
	 * SPE_TaskInfoList TaskInfoList=new SPE_TaskInfoList(); try{ try { String
	 * sSQL1=
	 * "SELECT RWH,RWLX,SJYBS,CLDXH,GYH FROM RW_RWXX WHERE RWLX=1 OR RWLX=2 AND GYH="
	 * ; String sSQL2=" ORDER BY RWH,SJYXH"; String
	 * sSqlStr=sSQL1+(""+ProtocolType)+sSQL2;
	 * 
	 * SFE_TaskInfo TaskInfo=new SFE_TaskInfo(); Connection conn =
	 * DriverManager.
	 * getConnection(IFE_FrameDataAreaExplain.FConnectString,"gd","gd");
	 * Statement stmt =
	 * conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet
	 * .CONCUR_READ_ONLY);//创建一个可以滚动的只读的SQL语句对象 ResultSet rset =
	 * stmt.executeQuery(sSqlStr); String sOldTaskNo=""; int iTaskCount=0;
	 * rset.next();//打开数据集 while (!rset.isAfterLast()) { if (rset.isFirst()){
	 * sOldTaskNo=rset.getString("RWH"); } if
	 * (sOldTaskNo.equals(rset.getString("RWH"))){
	 * TaskInfo.TaskInfoAdd(Integer.parseInt
	 * (sOldTaskNo),Integer.parseInt(rset.getString
	 * ("RWLX")),Integer.parseInt(rset
	 * .getString("GYH")),Integer.parseInt(rset.getString("CLDXH")));
	 * TaskInfo.CommandAdd(rset.getString("SJYBS")); if (rset.isLast()){
	 * TaskInfoList.TaskInfoList.add(TaskInfo); iTaskCount=iTaskCount+1; }
	 * rset.next(); } else { TaskInfoList.TaskInfoList.add(TaskInfo);
	 * TaskInfo=new SFE_TaskInfo(); iTaskCount=iTaskCount+1;
	 * sOldTaskNo=rset.getString("RWH"); } }
	 * TaskInfoList.SetFTaskCount(iTaskCount); stmt.close(); conn.close(); }
	 * catch (Exception e) {
	 * System.out.println("数据区解析InitialTaskInfoList出错:"+e.toString());
	 * Glu_ConstDefine.Log1.WriteLog(
	 * "报错函数:数据区解析FrameDataAreaExplainGluMethod__InitialTaskInfoList();报错原因:"
	 * +e.toString()); } } finally{ return TaskInfoList; } }
	 */
	public SFE_NormalData ExplainNormalPhysicalData(String CommandData,
		...
		try {
			try {
				int iDataItemCount = ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
						.get(CommandOrder))).GetFDataItemCount();
				int iDataItemLen = 0;
				...
				for (int i = 0; i < iDataItemCount; i++) {
					sDataFormat = ((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
							.get(CommandOrder))).DataItemInfoList.get(i))
							.GetFDataFormat().toUpperCase();
					if (sDataFormat.equals("X")) { // 不定长数据类型
						iDataItemLen = CommandData.length();
					} else {
						// 得到数据项字符长度=字节长度*2
						iDataItemLen = 2 * ((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
								.get(CommandOrder))).DataItemInfoList.get(i))
								.GetFDataLen();
					}
					// 得到数据项标识
					sDataCaption = ((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
							.get(CommandOrder))).DataItemInfoList.get(i))
							.GetFDataCaption();
					// 设置物理数据
					if (iDataItemLen <= CommandData.length()) { // 数据长度是否合法
						((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
								.get(CommandOrder))).DataItemInfoList.get(i))
								.SetFPhysicalData(CommandData.substring(0,
										iDataItemLen));
						// 物理数据转换成逻辑数据
						if (((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
								.get(CommandOrder))).DataItemInfoList.get(i))
								.PhysicalDataToLogicData()) {
							// 得到逻辑数据
							sLogicData = ((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
									.get(CommandOrder))).DataItemInfoList
									.get(i)).GetFLogicData();
						} else {
							sLogicData = "";
						}
				...
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainGluMethod__ExplainNormalPhysicalData();Error:"
								+ e.toString());
			}
		} finally {
		}
			...
			try {
				if (ProtocolType >= Glu_ConstDefine.GY_ZD_ZHEJIANG
					...
					for (int i = 0; i < 8; i++) {
						...
						|| ProtocolType == Glu_ConstDefine.GY_ZD_IHD) { // 全国规约、天津模块表
					...
					String sDA1 = MeasuredSign.substring(0, 2); // 信息点元
					int iDA2 = Integer.parseInt(MeasuredSign.substring(2, 4),
							16); // 信息点组
					if (sDA1.equals("00")) { // 测量点号为0
						iCount = 1;
						iMeasuredSignList[0] = 0;
					...
					int iObjectType = Integer.parseInt(
							MeasuredSign.substring(0, 2), 16) & 7; // 低3位表示对象类型
				 ...
				 * if (MeasuredSign.equals("0000")) { iCount = 1;
				 * iMeasuredSignList[0] = 0; } else { iCount = 1;
				 * iMeasuredSignList[0] = Integer.parseInt(MeasuredSign, 16); }
				 * }
				 */
			} catch (Exception e) {
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainGluMethod__GetMeasuredPointList();Error:"
								+ e.toString());
			}
		} finally {
		}
		...
		try {
			try {
				...
				for (int j = 0; j < iCommandCount; j++) {
					Command = ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
							.get(j))).GetFProtocolCommand();
					if (DataCaption.equals(Command)) { // 查找是否存在当前命令
						iCommandLen = 2 * ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
						...
						} else if (GYH == Glu_ConstDefine.GY_ZD_QUANGUO
								&& Count > 0) { // 谐波
							if (Fn == 58) { // 一类数据的F58很特殊，前3个数据项电压有总，后3个数据项电流没有总
								iCommandLen = 3 * Count * 4 + 3 * (Count - 1)
										* 4;
							} else {
						...
						if (iCommandLen <= FrameDataArea.length()) { // 如果任务数据长度合法
							...
							NormalDataTemp.SetDataItemCount(NormalData
									.GetDataItemCount());
							NormalDataTemp.DataItemList = NormalData.DataItemList;
						} else { // 数据长度不够
							iCommandLen = -1;
						}
				...
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainGluMethod__CommandSearchAndExplain();Error:"
								+ e.toString());
			}
		} finally {
		}
			...
			try {
				...
				for (int j = 0; j < iCommandCount; j++) {
					...
					if (DataCaption.equals(Command)) { // 查找是否存在当前命令
						try {
							int iDataItemCount = ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
									.get(j))).GetFDataItemCount();
							if (iDataItemCount > 1
									&& CommandData.substring(0, 2).equals("02")) {// 消去结构体类型及结构元素数
							...
							for (int i = 0; i < iDataItemCount; i++) {
								// 得到数据项字符长度
								int iDataType = ((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
										.get(j))).DataItemInfoList.get(i))
										.GetFDataType();
								switch (iDataType) {
								case 11:
								case 14:
								case 31: // delete datatype
									CommandData = CommandData.substring(2,
											CommandData.length());
									iDataItemLen = 2 * ((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
											.get(j))).DataItemInfoList.get(i))
											.GetFDataLen();
									iCommandLen = iCommandLen + 2
											+ iDataItemLen;
									break;
								case 12:
								case 21:
								case 30: // delete datatype and length
									if (CommandData.substring(0, 4).equals(
											"0D08")) {
										iDataItemLen = 6;
									} else {
										iDataItemLen = 2 * Integer
								...
								case 13:
								case 22: // no datatype
									CommandData = CommandData.substring(0,
											CommandData.length());
									iDataItemLen = 2 * ((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
											.get(j))).DataItemInfoList.get(i))
											.GetFDataLen();
								...
								if (iDataItemLen == 0) {
									sLogicData = "-2";
									NormalData.DataItemAdd(sDataCaption,
											sLogicData);
									NormalDataTemp.SetDataItemCount(NormalData
											.GetDataItemCount());
									NormalDataTemp.DataItemList = NormalData.DataItemList;
									...
									if (iDataType == 51) {// structure
										...
										for (int k = 0; k < iCount; k++) {
											...
											CommandData = ((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
													.get(j))).DataItemInfoList
													.get(i)).GetFPhysicalData();
											NormalData.DataItemAdd(
													sDataCaption, sLogicData);
											NormalDataTemp
													.SetDataItemCount(NormalData
															.GetDataItemCount());
											NormalDataTemp.DataItemList = NormalData.DataItemList;
										}
										iCommandLen = iFrameLen
												- CommandData.length(); // 命令实际长度
									} else {
										// 设置物理数据
										if (iDataItemLen <= CommandData
												.length()) { // 数据长度是否合法
											((SFE_DataItemInfo) ((SFE_CommandInfo) (CommandInfoList.CommandInfoList
													.get(j))).DataItemInfoList
													.get(i))
				...
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainGluMethod__DLMSCommandSearchAndExplain();Error:"
								+ e.toString());
			}
		} finally {
		}
	...
	public ArrayList<String> ExplainCommandList(String sDT, String sGNM) { // 解释国网命令列表
		...
		try {
			try {
				...
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainGluMethod__ExplainCommandList();Error:"
								+ e.toString());
			}
		} finally {
		}
	...
	public int GetGuYuanDataType(String DT, String FunctionCode) { // 得到数据类型:10普通数据;11中继数据(普通数据结构);21小时冻结数据;22曲线数据;23日冻结数据;24月冻结数据;25天津模块表主动上送数据;30告警数据;40设置返回数据
		...
		try {
			try {
				if (FunctionCode.equals("00")) { // 确认/否认
					...
					if (FunctionCode.equals("0C")) { // 一类数据
						...
						if ((iFn >= 101 && iFn <= 104) || iFn == 234) {
							iResult = 22; // 曲线数据
						} else if (((iFn == 49) || (iFn >= 201 && iFn <= 219) || (iFn == 235))) {
							iResult = 23; // 日冻结数据
						} else {
							iResult = 24; // 月冻结数据
						}
				...
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainGluMethod__GetQuanGuoDataType();Error:"
								+ e.toString());
			}
		} finally {
		}
	...
	public int GetHeXingDataType(String CommandID, String Content,
			String FunctionCode, String ControlCode) { // 得到数据类型:10普通数据;11中继数据(普通数据结构);21小时冻结数据;22曲线数据;23日冻结数据;24月冻结数据;25天津模块表主动上送数据;30告警数据;40设置返回数据
		int iResult = 0;
		try {
			if (ControlCode.substring(1, 2).equals("1")
					&& FunctionCode.equals("81")) { // 否认
				iResult = 40;
			} else if ((CommandID.equals("1011") || CommandID.equals("1012")
					|| CommandID.equals("1013") || CommandID.equals("1014")
					|| CommandID.equals("1041") || CommandID.equals("2001")
					|| CommandID.equals("2041") || CommandID.equals("2002")
					|| CommandID.equals("2042") || CommandID.equals("2040")
					|| CommandID.equals("2052") || CommandID.equals("1051"))
					&& (Content.equals(""))) {// 设置成功
				iResult = 40;
				...
				iResult = 22; // 曲线数据
			} else if (CommandID.equals("2004") || CommandID.equals("2044")
					|| CommandID.equals("2020") || CommandID.equals("204B")
					|| CommandID.equals("200A") || CommandID.equals("202A")) {
				iResult = 23; // 日冻结数据
			} else if (CommandID.equals("2005") || CommandID.equals("2045")
					|| CommandID.equals("2021") || CommandID.equals("204C")
					|| CommandID.equals("200B") || CommandID.equals("202B")) {
				iResult = 24; // 月冻结数据
			} else if (CommandID.equals("1015") || CommandID.equals("3002")
					|| CommandID.equals("3003") || CommandID.equals("3004")
					|| CommandID.equals("3005") || CommandID.equals("2018")
					|| CommandID.equals("2048") || CommandID.equals("2009")
					|| CommandID.equals("2019") || CommandID.equals("204A")) {
				iResult = 25;
			...
			Glu_ConstDefine.Log1
					.WriteLog("Func:FrameDataAreaExplainGluMethod__GetHeXingDataType();Error:"
							+ e.toString());
		}
		return iResult;
	}
	...
	public int GetQuanGuo698DataType(String DT, String FunctionCode) { // 得到数据类型:10普通数据;11中继数据(普通数据结构);21小时冻结数据;22曲线数据;23日冻结数据;24月冻结数据;25天津模块表主动上送数据;30告警数据;40设置返回数据
		...
		try {
			try {
				if (FunctionCode.equals("00")) { // 确认/否认
					iResult = 40;
				} else if ((FunctionCode.equals("0A"))
						|| (FunctionCode.equals("03"))
					...
					if (FunctionCode.equals("0C")) { // 一类数据
						...
						if ((iFn >= 73 && iFn <= 110)
								|| (iFn >= 138 && iFn <= 148)
								|| (iFn >= 217 && iFn <= 218)
								|| (iFn >= 401 && iFn <= 716) || (iFn == 393)
								|| (iFn >= 396 && iFn <= 399)) {
							iResult = 22; // 曲线数据
						} else if ((iFn >= 1 && iFn <= 8)
								|| (iFn >= 25 && iFn <= 32)
								|| (iFn >= 41 && iFn <= 43)
								|| (iFn >= 49 && iFn <= 50) || (iFn == 53)
								|| (iFn >= 57 && iFn <= 59)
								|| (iFn >= 113 && iFn <= 129)
								|| (iFn >= 153 && iFn <= 156)
								|| (iFn >= 161 && iFn <= 168)
								|| (iFn >= 185 && iFn <= 188) || (iFn == 209)) {
							iResult = 23; // 日冻结数据
						} else if ((iFn >= 9 && iFn <= 12)
								|| (iFn >= 169 && iFn <= 176)
								|| (iFn >= 189 && iFn <= 192) || (iFn == 394)
								|| (iFn == 395)) {
							iResult = 26; // 抄表日冻结数据
						} else {
							iResult = 24; // 月冻结数据
				...
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainGluMethod__GetQuanGuo698DataType();Error:"
								+ e.toString());
			}
		} finally {
		}
					...
					if (FunctionCode.equals("0C")) { // 一类数据
						...
						if ((iFn >= 73 && iFn <= 108) || iFn == 138
							...
							iResult = 22; // 曲线数据
								...
								|| (iFn >= 25 && iFn <= 31) || (iFn >= 41)
								&& (iFn <= 43) || (iFn >= 49 && iFn <= 50)
								|| (iFn >= 57 && iFn <= 59) || (iFn >= 113 && iFn <= 130))) {
							iResult = 23; // 日冻结数据
						} else {
							iResult = 24; // 月冻结数据
						}
			...
			try {
				if (FunctionCode.equals("00")) { // 确认/否认
					iResult = 40;
						...
						|| FunctionCode.equals("0C")) { // 读取终端、测量点参数、一类当前数据
					iResult = 10;
				...
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainGluMethod__GetIHDDataType();Error:"
								+ e.toString());
			}
		} finally {
		}
	...
	public int GetSelfDefineTaskNo(int GYH, String sCommand,
		...
		for (int i = 0; i < TaskInfoList.TaskInfoList.size(); i++) {
			int iTaskType = ((SFE_TaskInfo) (TaskInfoList.TaskInfoList.get(i)))
					.GetFTaskType();
			int iCommandFindCount = 0;
			if (iTaskType > 1) {
				String sDataCaption = ((String) ((SFE_TaskInfo) (TaskInfoList.TaskInfoList
						.get(i))).CommandList.get(0));
				int TaskType = ((SFE_TaskInfo) (TaskInfoList.TaskInfoList
						.get(i))).GetFTaskType();
				if (GYH != Glu_ConstDefine.GY_ZD_HEXING) {
					int iCommand = Integer.parseInt(
							sCommand.substring(1, sCommand.length()), 16);
					int iCaption = Integer.parseInt(
							sDataCaption.substring(1, sDataCaption.length()),
							16);
					// 曲线当前数据需要完全匹配数据项，区别前几日曲线数据
					/*
					 * if ((GYH == Glu_ConstDefine.GY_ZD_TIANJIN) &&
					 * (sCommand.substring(0, 1).equals("D")) &&
					 * (sCommand.substring(3, 4).equals("0"))) { if
					 * (sDataCaption.equals(sCommand)) { iCommandFindCount = 1;
					 * } else { iCommandFindCount = 0; } } else if ((GYH ==
					 * Glu_ConstDefine.GY_ZD_TIANJIN) &&
					 * (sDataCaption.substring(0,
					 * 3).equals(sCommand.substring(0, 3)))) { iCommandFindCount
					 * = 1; } else if ((GYH == Glu_ConstDefine.GY_ZD_GUYUAN) &&
					 * (sDataCaption.equals(sCommand))) { //跟数据库命令匹配就累加1
					 * iCommandFindCount = 1; } else
					 */if ((GYH == Glu_ConstDefine.GY_ZD_QUANGUO)
							&& (sDataCaption.equals(sCommand))) { // 跟数据库命令匹配就累加1
						iCommandFindCount = 1;
					} else if ((TaskType == 1301)
							&& (GYH == 105)
							&& (sCommand.substring(0, 1).equals(sDataCaption
									.substring(0, 1)))
	...
	@SuppressWarnings("rawtypes")
	public ArrayList<SFE_HistoryData> SearchAndMatchingTaskInfoList(
			ArrayList CommandListTemp, SFE_HistoryData[] HistoryDataListTemp,
			int HistoryDataCount, SPE_TaskInfoList TaskInfoList) {
		ArrayList<SFE_HistoryData> HistoryDataList = new ArrayList<SFE_HistoryData>();
		try {
			try {
				// 从任务信息队列查找任务号和测量点类型
				int iTaskDataItemCount = 0, iTaskNo = 0, iCommandFindCount = 0, iTaskType = 0, iMeasuredPointNo = 0;
				String sCommand = "", sDataCaption = "", sTaskDateTime = "";
				SFE_HistoryData HistoryData = new SFE_HistoryData();
				for (int i = 0; i < TaskInfoList.TaskInfoList.size(); i++) {
					iTaskType = ((SFE_TaskInfo) (TaskInfoList.TaskInfoList
							.get(i))).GetFTaskType();
					iMeasuredPointNo = ((SFE_TaskInfo) (TaskInfoList.TaskInfoList
							.get(i))).GetFMeasuredPointNo();
					if (iTaskType != 10) {
						iTaskDataItemCount = ((SFE_TaskInfo) (TaskInfoList.TaskInfoList
								.get(i))).GetFDataItemCount();
						if (iTaskDataItemCount == CommandListTemp.size()) { // 找到跟解释出来的命令数目一致的任务
							iCommandFindCount = 0;
							for (int j = 0; j < iTaskDataItemCount; j++) {
								sDataCaption = ((String) ((SFE_TaskInfo) (TaskInfoList.TaskInfoList
										.get(i))).CommandList.get(j));
								for (int k = 0; k < CommandListTemp.size(); k++) {
									sCommand = CommandListTemp.get(k)
											.toString();
									if (sDataCaption.equals(sCommand)) { // 跟数据库命令匹配就累加1
										iCommandFindCount = iCommandFindCount + 1;
									}
								}
							}
							if (iTaskDataItemCount == iCommandFindCount
									&& (HistoryDataListTemp[0]
											.GetMeasuredPointNo() == iMeasuredPointNo || iMeasuredPointNo == -1)) { // 如果数据库配置的命令跟解释后的命令都匹配
								iTaskNo = ((SFE_TaskInfo) (TaskInfoList.TaskInfoList
										.get(i))).GetFTaskNo();
								for (int j = 0; j < HistoryDataCount; j++) {
									...
									HistoryData
											.SetMeasuredPointNo(HistoryDataListTemp[j]
													.GetMeasuredPointNo()); // 测量点号取自解释出来的测量点号
									HistoryData.SetMeasuredAdd(new String(
											HistoryDataListTemp[j]
													.GetMeasuredAdd())); // 测量点号取自解释出来的测量点地址
									HistoryData
											.SetMeasuredPointType(HistoryDataListTemp[j]
													.GetMeasuredPointType());
									for (int k = 0; k < HistoryDataListTemp[j]
											.GetDataItemCount(); k++) {
										HistoryData.DataItemList
												.add(HistoryDataListTemp[j].DataItemList
														.get(k));
				...
				Glu_ConstDefine.Log1
						.WriteLog("Func:FrameDataAreaExplainQuanGuo__SearchQuanGuoTaskInfoAndReturnHistoryDataList();Error:"
								+ e.toString());
			}
		} finally {
		}
